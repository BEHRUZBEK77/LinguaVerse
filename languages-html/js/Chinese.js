...
// =====================================================
// Chinese.js — LinguaVerse (Xitoy tili kursi)
...
const UD_DATA = {
    beginner: [
        {
            id: 'cb0', emoji: '<i class="fa-solid fa-font" style="color:#60a5fa"></i>', title: '拼音入门 (Pinyin)', desc: "Xitoy pinyin alifbosini o'rganish",
        ...
        {
            id: 'cb1', emoji: '<i class="fa-solid fa-hand" style="color:#f5c842"></i>', title: '问候 (Greetings)', desc: "Salomlashish va xayrlashish",
        ...
        {
            id: 'cb2', emoji: '<i class="fa-solid fa-hashtag" style="color:#60a5fa"></i>', title: '数字 (Numbers)', desc: "Xitoy raqamlari",
        ...
        {
            id: 'cb3', emoji: '<i class="fa-solid fa-palette" style="color:#a78bfa"></i>', title: '颜色 (Colors)', desc: "Xitoy tilida ranglar",
        ...
        {
            id: 'cb4', emoji: '<i class="fa-solid fa-people-roof" style="color:#60a5fa"></i>', title: '家人 (Family)', desc: "Oila a'zolari",
        ...
        {
            id: 'cb5', emoji: '<i class="fa-solid fa-bowl-food" style="color:#fb923c"></i>', title: '食物饮料 (Food)', desc: "Xitoy yemliklari va ichimliklar",
    ...
    elementary: [
        {
            id: 'ce1', emoji: '<i class="fa-solid fa-house-chimney" style="color:#5b7cfa"></i>', title: '家和房间 (House)', desc: "Uy va xonalar",
        ...
        {
            id: 'ce2', emoji: '<i class="fa-solid fa-briefcase" style="color:#94a3b8"></i>', title: '职业 (Jobs)', desc: "Kasblar",
        ...
        {
            id: 'ce3', emoji: '<i class="fa-solid fa-cart-shopping" style="color:#5b7cfa"></i>', title: '购物 (Shopping)', desc: "Xarid qilish",
    ...
    'pre-intermediate': [
        {
            id: 'cp1', emoji: '<i class="fa-solid fa-hat-wizard" style="color:#a78bfa"></i>', title: '将来计划 (Future Plans)', desc: "Kelajak rejalari",
        ...
        {
            id: 'cp2', emoji: '<i class="fa-solid fa-bullseye" style="color:#f87171"></i>', title: '是非句 (Yes/No Questions)', desc: "Xitoy tilida savol gaplari",
            level: 'pre-intermediate', words: ['然而', '虽然', '因此', '而且', '机会', '挑战', '成功', '放弃', '实现', '发展'],
            xp: 140, coin: 65,
    ...
    advanced: [
        {
            id: 'ca1', emoji: '<i class="fa-solid fa-pen" style="color:#60a5fa"></i>', title: '学术写作 (Academic Writing)', desc: "Akademik yozuv",
        ...
        {
            id: 'ca2', emoji: '<i class="fa-solid fa-clipboard-list" style="color:#94a3b8"></i>', title: 'HSK写作 (HSK Writing)', desc: "HSK yozuv darsi",
            level: 'advanced', words: ['然而', '而且', '因此', '细微差别', '雄辩', '减轻', '史无前例', '一丝不苟', '机会', '虽然'],
            xp: 250, coin: 110,
    ...
    try {
            ...
            html += `<div class="lb-row${isMe ? ' me' : ''}" style="${isMe ? 'background:rgba(91,124,250,0.1);border-color:rgba(91,124,250,0.28);' : ''}">
                ...
                <div style="flex:1;min-width:0">
                    <div style="font-weight:700;font-size:0.88rem;color:#e8ecff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
                        ${esc(u.displayName || u.email || 'Foydalanuvchi')}
        ...
        if (e.code === 'failed-precondition' || e.message?.includes('index')) {
            container.innerHTML = `<div style="text-align:center;padding:20px;color:#f5c842;font-size:0.82rem">
                <i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Bu maydon uchun Firestore index yo'q. XP bo'yicha yuklanmoqda...
            </div>`;
...
window.openUnit = function (unit) {
    ...
    content.innerHTML = `
      ...
      <div>
        <div style="font-size:0.78rem;color:#666;margin-bottom:8px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> So'zlar (汉字):</div>
...
function lessonA(unit) {
    ...
    const fills = words.slice(0, 4).map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px">
            ...
            <button onclick="window.aiExWord('${w}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI</button>
            <button onclick="window.spk('${w}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
          </div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> 汉字 Lug'at</h3>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px">
        ...
        return `<div style="padding:14px;border-radius:12px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06)">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:2px">
                ...
                <button onclick="window.spk('${w}',event)" style="background:none;border:none;cursor:pointer;font-size:1rem"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
              </div>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(239,68,68,0.05);border:1px solid rgba(239,68,68,0.15);border-radius:12px">
      ...
      <div style="font-size:0.9rem;color:#fca5a5;margin-bottom:8px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${unit.grammar_rule || ''}</div>
      <div style="font-size:0.85rem;color:#ef4444;font-style:italic"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> ${unit.grammar_example || ''}</div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-puzzle-piece" style="color:#a78bfa"></i> Juftlash (汉字 <i class="fa-solid fa-left-right"></i> O'zbek)</h3>
    ...
    <button onclick="window.finLessonA('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#ef4444,#f97316);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Grammatika darsini yakunlash</button>`;
}
...
function lessonB(unit) {
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Diktant (听写)</h3>
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playDict('${unit.id}','slow')" style="padding:8px 16px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Sekin</button>
      </div>
      ...
      <div style="display:flex;gap:6px;margin-top:8px">
        ...
        <button onclick="window.aiChkDict()" style="padding:7px 14px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI tahlil (1 token)</button>
      </div>
      <div id="dictFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonB('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#22d3ee,#ef4444);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Listening yakunlash</button>`;
}
...
function renderLex(exs, idx) {
    ...
    return `<div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playLex(${idx},'slow')" style="padding:8px 16px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Sekin</button>
      </div>
...
function lessonC(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-book-open" style="color:#a78bfa"></i> Matn o'qish (阅读)</h3>
      <div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px;margin-top:12px">
          <button onclick="window.rdAloud()" style="padding:7px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i> Tinglash (中文)</button>
        </div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-circle-question" style="color:#93c5fd"></i> Savolar (问题)</h3>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-font" style="color:#60a5fa"></i> Pinyin yozish</h3>
      ${wh.map((w, i) => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap">
          ...
          <button onclick="window.spk('${w}',event)" style="padding:7px 12px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
...
window.chkWH = function (i) {
    ...
    if (userNoSpace === correct || inp.value.trim().toLowerCase() === inp.dataset.ans.toLowerCase()) {
        fb.innerHTML = '<i class="fa-solid fa-circle-check" style="color:#34d399"></i>'; inp.style.borderColor = '#34d399'; awardXP(8, 'reading');
    } else { fb.innerHTML = `<i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> <span style="color:#666;font-size:0.7rem">${inp.dataset.ans}</span>`; inp.style.borderColor = '#ef4444'; }
};
...
function lessonD(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Speaking (口语)</h3>
      ${topics.map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px;margin-bottom:8px">
            ...
            <button onclick="window.spk('${w}',event)" style="padding:8px 12px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
          </div>
          ...
          <div style="display:flex;gap:6px;margin-top:8px">
            <button onclick="window.aiSpk(${i},'${w}')" style="padding:6px 14px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI baholash (1 token)</button>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Writing (写作)</h3>
      <div style="padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px">
          <button onclick="window.aiWrit('${unit.title.replace(/'/g, "\\'")}','${unit.words.slice(0, 5).join(',')}')" style="padding:7px 14px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI (1 token)</button>
          <button onclick="window.selfChk(40)" style="padding:7px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-chart-pie" style="color:#60a5fa"></i> Belgi soni</button>
        </div>
        <div id="wfb" style="margin-top:8px;font-size:0.8rem"></div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-shuffle" style="color:#5b7cfa"></i> Jumla tartibi (排列句子)</h3>
      ...
      <div style="display:flex;gap:6px">
        ...
        <button onclick="window.spk('${woSent}',event)" style="padding:7px 14px;border-radius:8px;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);color:#ef4444;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
      </div>
      <div id="wofb" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonD('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#f472b6,#ef4444);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Speaking & Writing yakunlash</button>`;
}
...
window.chkWO = function () {
    const fb = $id('wofb');
    if (!woAns.length) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Belgilarni tartibga qo\'ying!</span>'; return; }
...
window.selfChk = function (min) {
    ...
    const len = ta.value.replace(/\s/g, '').length;
    if (len >= min) { fb.innerHTML = `<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${len} belgi!</span>`; lScore++; awardXP(15, 'writing'); }
    else { fb.innerHTML = `<span style="color:#f87171"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> ${min - len} belgi kam!</span>`; }
    lTotal++;
};
...
async function saveLessonCompletion(unitId, lessonKey, score, total, xpEarned, coinEarned) {
    ...
    try {
        ...
        updateDisplays(); showToast(`<i class="fa-solid fa-circle-check" style="color:#34d399"></i> +${xpEarned} XP +${coinEarned} <i class="fa-solid fa-coins" style="color:#f5c842"></i> saqlandi!`, 'success');
...
function showResult(lk, pct, xp, coin, unit, uid) {
    ...
    content.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="font-size:1.2rem;margin-bottom:20px">${pct >= 80 ? '<i class="fa-solid fa-trophy" style="color:#f5c842"></i> 太棒了! Mukammal!' : pct >= 60 ? '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> 很好! Yaxshi!' : '<i class="fa-solid fa-dumbbell" style="color:#34d399"></i> 再试一次! Qayta urining!'}</div>
...
function renderWords(reset = true) {
    ...
    slice.slice(wOff).forEach(w => {
        ...
        card.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:2px">
          ...
          <button onclick="window.spk('${w.e}',event)" style="background:none;border:none;cursor:pointer;font-size:1rem"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
        </div>
...
function openWModal(w) {
    ...
    c.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="display:flex;gap:8px;justify-content:center">
        <button onclick="window.spk('${w.e}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.3);color:#fca5a5;cursor:pointer;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i> Talaffuz</button>
        <button onclick="window.aiExWord('${w.e}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.25);color:#a78bfa;cursor:pointer;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI (1 token)</button>
      </div>
...
window.checkQuizOpt = function (btn, chosen, type) {
    ...
    else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: ${curQuizWord.e} (${curQuizWord.py}) = ${curQuizWord.uz}</span>`; }
...
window.selectMatch2 = function (el) {
    ...
    if (!matchSel1) {
        ...
        if (matchSel1.dataset.id === el.dataset.id) {
            ...
            if (matchMatched.length === matchPairs.length) { const fb = $id('matchFeedback'); if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Barcha juftliklar! 棒极了!</span>'; }
        } else {
...
window.checkTyping = function () {
    ...
    if (val === w.e) {
        if (fb) fb.innerHTML = `<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri! ${w.e} (${w.py})</span>`;
...
window.clearChatHistory = async function () {
    ...
    if (c) c.innerHTML = `<div class="chat-msg ai-msg"><div class="chat-avatar">🈶</div><div class="chat-bubble">你好！Chat tarixi tozalandi. 开始新的对话！<i class="fa-solid fa-face-smile" style="color:#f5c842"></i></div></div>`;
