...
const UD_DATA = {
    beginner: [
        {
            id: 'b0', emoji: '<i class="fa-solid fa-font" style="color:#60a5fa"></i>', title: 'Alphabet A-Z', desc: "Ingliz alifbosini o'rganish", level: 'beginner',
        ...
        {
            id: 'b1', emoji: '<i class="fa-solid fa-hand" style="color:#f5c842"></i>', title: 'Greetings', desc: "Salomlashish va xayrlashish", level: 'beginner',
        ...
        {
            id: 'b2', emoji: '<i class="fa-solid fa-hashtag" style="color:#60a5fa"></i>', title: 'Numbers', desc: "Raqamlar va hisoblash", level: 'beginner',
        ...
        {
            id: 'b3', emoji: '<i class="fa-solid fa-palette" style="color:#a78bfa"></i>', title: 'Colors', desc: "Ingliz tilida ranglar", level: 'beginner',
        ...
        {
            id: 'b4', emoji: '<i class="fa-solid fa-people-roof" style="color:#60a5fa"></i>', title: 'Family', desc: "Oila a'zolari", level: 'beginner',
        ...
        {
            id: 'b5', emoji: '<i class="fa-solid fa-apple-whole" style="color:#f87171"></i>', title: 'Food & Drinks', desc: "Ovqat va ichimliklar", level: 'beginner',
    ...
    elementary: [
        {
            id: 'e1', emoji: '<i class="fa-solid fa-house-chimney" style="color:#5b7cfa"></i>', title: 'House & Rooms', desc: "Uy va xonalar", level: 'elementary',
        ...
        {
            id: 'e2', emoji: '<i class="fa-solid fa-briefcase" style="color:#94a3b8"></i>', title: 'Jobs', desc: "Kasblar", level: 'elementary',
        ...
        {
            id: 'e3', emoji: '<i class="fa-solid fa-cart-shopping" style="color:#5b7cfa"></i>', title: 'Shopping', desc: "Xarid qilish", level: 'elementary',
    ...
    'pre-intermediate': [
        {
            id: 'p1', emoji: '<i class="fa-solid fa-hat-wizard" style="color:#a78bfa"></i>', title: 'Future Plans', desc: "Kelajak zamonlari", level: 'pre-intermediate',
        ...
        {
            id: 'p2', emoji: '<i class="fa-solid fa-bullseye" style="color:#f87171"></i>', title: 'Present Perfect', desc: "Present Perfect zamon", level: 'pre-intermediate',
            words: ['however', 'although', 'therefore', 'moreover', 'opportunity', 'nuance', 'eloquence', 'mitigate', 'unprecedented', 'meticulous'],
            xp: 140, coin: 65,
    ...
    advanced: [
        {
            id: 'a1', emoji: '<i class="fa-solid fa-pen" style="color:#60a5fa"></i>', title: 'Academic Writing', desc: "Akademik yozuv", level: 'advanced',
        ...
        {
            id: 'a2', emoji: '<i class="fa-solid fa-clipboard-list" style="color:#94a3b8"></i>', title: 'IELTS Writing', desc: "IELTS Yozuv", level: 'advanced',
            words: ['moreover', 'however', 'therefore', 'nuance', 'eloquence', 'mitigate', 'unprecedented', 'meticulous', 'opportunity', 'although'],
            xp: 250, coin: 110,
...
// ══════════════════════════════════════════════════════════════
const GRAMMAR_QS = [
    { q: "She ___ to school every day.", opts: ["go", "goes", "going", "gone"], ans: "goes", exp: "3-shaxs birlik: go <i class='fa-solid fa-arrow-right'></i> goes" },
    ...
    { q: "This is ___ umbrella.", opts: ["a", "an", "the", "—"], ans: "an", exp: "Unli tovush oldidan 'an': umbrella <i class='fa-solid fa-arrow-right'></i> an" },
...
function updateDisplays() {
    ...
    if (navUInfo) navUInfo.innerHTML = `<div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="font-size:0.78rem;color:#f5c842"><i class="fa-solid fa-star" style="color:#f5c842"></i> ${UXP.toLocaleString()}</span>
        <span style="font-size:0.78rem;color:#fbbf24"><i class="fa-solid fa-coins" style="color:#f5c842"></i> ${UCoin.toLocaleString()}</span>
    </div>`;
}
    ...
    try {
        ...
        users.forEach((u, i) => {
            ...
            html += `<div class="lb-row${isMe ? ' me' : ''}">
                ...
                <div style="flex:1">
                    <div class="lb-name">${esc(u.displayName || u.email || 'Foydalanuvchi')}${isMe ? ' <span style="color:#a78bfa;font-size:0.72rem">(siz)</span>' : ''}</div>
                    ${planKey !== 'free' ? `<span class="lb-plan" style="border-color:${pc}44;color:${pc}">${esc(planLabel)}</span>` : ''}
                </div>
...
async function callAI(prompt, maxTok = 1000) {
    try {
        ...
        if (!r.ok) { const errText = await r.text(); return `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> AI xatolik: ${r.status}.`; }
    ...
    units.forEach((unit, i) => {
        ...
        card.innerHTML = `
          ...
          <div style="font-size:0.76rem;display:flex;gap:10px">
            ...
            <span style="color:#fbbf24">+${unit.coin} <i class="fa-solid fa-coins" style="color:#f5c842"></i></span>
            ${isComp ? '<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i></span>' : ''}
          </div>`;
...
window.openUnit = function (unit) {
    ...
    const lnames = { A: "<i class='fa-solid fa-book-open' style='color:#a78bfa'></i> Grammatika & Lug'at", B: "<i class='fa-solid fa-headphones' style='color:#5b7cfa'></i> Listening", C: "<i class='fa-solid fa-book-open' style='color:#a78bfa'></i> Reading", D: "<i class='fa-solid fa-microphone' style='color:#f87171'></i> Speaking & Writing" };
    ...
    content.innerHTML = `
      <div style="text-align:center;padding-bottom:20px">
        ...
        <div style="display:flex;gap:16px;justify-content:center;margin:16px 0;flex-wrap:wrap">
          <span style="color:#f5c842"><i class="fa-solid fa-star" style="color:#f5c842"></i> +${unit.xp} XP</span>
          <span style="color:#fbbf24"><i class="fa-solid fa-coins" style="color:#f5c842"></i> +${unit.coin} Coin</span>
          <span style="color:#60a5fa"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> ${unit.words.length} so'z</span>
          <span style="color:#a78bfa"><i class="fa-solid fa-ticket" style="color:#a78bfa"></i> 2 token/dars</span>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px">
        ...
        return `<div onclick="window.openLesson('${unit.id}','${k}')" style="padding:16px;border-radius:12px;background:${done ? 'rgba(52,211,153,0.08)' : 'rgba(255,255,255,0.03)'};border:1px solid ${done ? 'rgba(52,211,153,0.3)' : 'rgba(255,255,255,0.08)'};cursor:pointer;transition:all 0.2s" onmouseover="this.style.borderColor='${lcolors[k]}55'" onmouseout="this.style.borderColor='${done ? 'rgba(52,211,153,0.3)' : 'rgba(255,255,255,0.08)'}'">
                ...
                ${done ? `<div style="font-size:0.72rem;color:#34d399;margin-top:4px"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${sc}%</div>` : '<div style="font-size:0.72rem;color:#666;margin-top:4px">▶ Boshlash</div>'}
            </div>`;
    }).join('')}
      </div>
      <div>
        <div style="font-size:0.78rem;color:#666;margin-bottom:8px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> So'zlar:</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${unit.words.map(w => `<span onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.2);color:#c4b5fd;padding:4px 10px;border-radius:20px;font-size:0.76rem;cursor:pointer">${w} <i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></span>`).join('')}
        </div>
      </div>`;
    modal.classList.add('active');
...
function showLessonModal(unit, lk) {
    ...
    content.innerHTML = `<div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.08)">
        <button onclick="window.openUnit(window.__curUnit)" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#e8ecff;padding:6px 12px;border-radius:8px;cursor:pointer;font-size:0.8rem;font-family:inherit"><i class="fa-solid fa-arrow-left"></i> Orqaga</button>
...
function lessonA(unit) {
    ...
    const fills = words.slice(0, 4).map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px">
            <button onclick="window.chkFill(${i})" style="padding:6px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
            <button onclick="window.aiExWord('${w.replace(/'/g, "\\'")}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.25);color:#a78bfa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI</button>
            <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
          </div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> So'zlar lug'ati</h3>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(91,124,250,0.06);border:1px solid rgba(91,124,250,0.15);border-radius:12px">
      <h3 style="margin-bottom:10px;color:#e8ecff"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> Grammatika Qoidasi</h3>
      <div style="font-size:0.9rem;color:#c7d2fe;margin-bottom:8px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${unit.grammar_rule || ''}</div>
      <div style="font-size:0.85rem;color:#a78bfa;font-style:italic"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> ${unit.grammar_example || ''}</div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> To'ldirish Mashqlari</h3>
      ${fills}
      <div id="vocabAIFB" style="font-size:0.8rem"></div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-puzzle-piece" style="color:#a78bfa"></i> Juftlash Mashqi</h3>
    ...
    <button onclick="window.finLessonA('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#5b7cfa,#a78bfa);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Grammatika darsini yakunlash</button>`;
}
...
window.chkFill = function (i) {
    ...
    if (inp.value.trim().toLowerCase() === inp.dataset.ans.toLowerCase()) {
        fb.innerHTML = `<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri! "${inp.dataset.ans}"</span>`;
        inp.style.borderColor = '#34d399'; lScore++; awardXP(10, 'grammar');
    } else {
        fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri javob: <strong>${inp.dataset.ans}</strong></span>`;
...
window.aiGrammarExplain = async function (title, rule) {
    ...
    const r = await callAI(`"${title}" mavzusida "${rule}" grammatika qoidasini ${NATIVE_LANG} tilida (javob shu tilda bo'lsin) tushuntir. 3 ta misol keltir.`, 800);
...
window.aiExWord = async function (word, e) {
    ...
    const r = await callAI(`"${word}" inglizcha so'zini ${NATIVE_LANG} tilida (javob shu tilda bo'lsin): 1) Ma'nosi 2) 3 misol 3) Eslatma`, 600);
    if (fb) fb.innerHTML = esc(r).replace(/\n/g, '<br>');
};
...
function lessonB(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-headphones" style="color:#5b7cfa"></i> Listening Mashqi</h3>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Diktant</h3>
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playDict('${unit.id}','slow')" style="padding:8px 16px;border-radius:8px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.2);color:#a78bfa;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Sekin</button>
      </div>
      ...
      <div style="display:flex;gap:6px;margin-top:8px">
        <button onclick="window.chkDict()" style="padding:7px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
        <button onclick="window.aiChkDict()" style="padding:7px 14px;border-radius:8px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.25);color:#a78bfa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI tahlil (1 token)</button>
      </div>
      <div id="dictFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonB('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#22d3ee,#5b7cfa);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Listening yakunlash</button>`;
}
...
function renderLex(exs, idx) {
    const ex = exs[idx];
    if (!ex) return '<div style="text-align:center;padding:20px;color:#34d399"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Barcha listening mashqlari tugadi!</div>';
    return `<div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playLex(${idx},'slow')" style="padding:8px 16px;border-radius:8px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.2);color:#a78bfa;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Sekin</button>
      </div>
      ...
      <div style="display:flex;gap:6px">
        <button onclick="window.chkLex(${idx},${ex.c})" style="padding:7px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
        ${idx + 1 < exs.length ? `<button onclick="window.nextLex(${idx + 1})" id="lexnxt${idx}" style="display:none;padding:7px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-arrow-right"></i> Keyingi</button>` : ''}
      </div>
...
window.chkLex = function (idx, correct) {
    ...
    if (sel === undefined) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Javob tanlang!</span>'; return; }
    ...
    if (sel === correct) { if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> To\'g\'ri!</span>'; lScore++; awardXP(15, 'listening'); }
    else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: <strong>${String.fromCharCode(65 + correct)}</strong></span>`; }
    lTotal++;
...
window.chkDict = function () {
    ...
    if (!inp || !fb || !dictSent) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Avval audio tinglang!</span>'; return; }
    ...
    fb.innerHTML = `<div><strong>To'g'ri:</strong> ${hl}</div><div style="margin-top:6px"><strong>Siz:</strong> ${esc(inp.value)}</div><div style="font-size:0.9rem;font-weight:700;margin-top:6px;color:${pct >= 70 ? '#34d399' : '#ef4444'}">${pct}%</div>`;
...
function lessonC(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-book-open" style="color:#a78bfa"></i> Matn o'qish</h3>
      <div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px;margin-top:12px">
          <button onclick="window.rdAloud()" style="padding:7px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i> Tinglash</button>
        </div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-circle-question" style="color:#93c5fd"></i> Savolar</h3>
      ...
      <button onclick="window.chkAllRQ(${JSON.stringify(qs.map(q => q.c))})" style="padding:8px 16px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.82rem;font-family:inherit;margin-top:8px"><i class="fa-solid fa-check" style="color:#34d399"></i> Hammasini tekshir</button>
      <div id="rdTotFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-font" style="color:#60a5fa"></i> So'z yozish</h3>
      ${wh.map((w, i) => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          ...
          <button onclick="window.chkWH(${i})" style="padding:7px 12px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i></button>
          <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:7px 12px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
          <span id="whfb${i}" style="font-size:0.8rem;min-width:30px"></span>
        </div>`;
    }).join('')}
    </div>
    <button onclick="window.finLessonC('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#34d399,#22d3ee);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Reading yakunlash</button>`;
}
...
window.chkAllRQ = function (answers) {
    ...
    answers.forEach((correct, qi) => {
        ...
        if (sel === correct) { score++; if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> To\'g\'ri!</span>'; }
        else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: ${String.fromCharCode(65 + correct)}</span>`; }
    });
...
window.chkWH = function (i) {
    ...
    if (inp.value.trim().toLowerCase() === inp.dataset.ans.toLowerCase()) {
        fb.innerHTML = '<i class="fa-solid fa-circle-check" style="color:#34d399"></i>'; inp.style.borderColor = '#34d399'; awardXP(8, 'writing');
...
function lessonD(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Speaking Mashqi</h3>
      ${topics.map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px;margin-bottom:8px">
            <button id="mbtn${i}" onclick="window.togMic(${i})" style="padding:8px 16px;border-radius:8px;background:rgba(244,114,182,0.1);border:1px solid rgba(244,114,182,0.25);color:#f472b6;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Gapirish</button>
            <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:8px 12px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
          </div>
          ...
          <div style="display:flex;gap:6px;margin-top:8px">
            <button onclick="window.aiSpk(${i},'${w}')" style="padding:6px 14px;border-radius:8px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.25);color:#a78bfa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI baholash (1 token)</button>
            <button onclick="window.markDone(${i})" style="padding:6px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Bajarildi</button>
          </div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Writing Mashqi</h3>
      <div style="padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px">
          <button onclick="window.aiWrit('${unit.title}','${unit.words.slice(0, 5).join(',')}')" style="padding:7px 14px;border-radius:8px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.25);color:#a78bfa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI (1 token)</button>
          <button onclick="window.selfChk(40)" style="padding:7px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-chart-pie" style="color:#60a5fa"></i> So'z soni</button>
        </div>
        <div id="wfb" style="margin-top:8px;font-size:0.8rem"></div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-shuffle" style="color:#5b7cfa"></i> So'z Tartibi</h3>
      ...
      <div style="display:flex;gap:6px">
        <button onclick="window.chkWO()" style="padding:7px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
        <button onclick="window.rstWO()" style="padding:7px 14px;border-radius:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:#e8ecff;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Qayta</button>
        <button onclick="window.spk('${woSent.replace(/'/g, "\\'")}',event)" style="padding:7px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
      </div>
      <div id="wofb" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonD('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#f472b6,#a78bfa);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Speaking & Writing yakunlash</button>`;
}
...
window.chkWO = function () {
    const fb = $id('wofb');
    if (!woAns.length) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> So\'zlarni tartibga qo\'ying!</span>'; return; }
    if (woAns.join(' ').toLowerCase() === (window.__woCorrect || '').toLowerCase()) {
        if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-trophy" style="color:#f5c842"></i> Mukammal!</span>';
        awardXP(15, 'writing'); lScore++;
    } else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: <em>${window.__woCorrect}</em></span>`; }
    lTotal++;
};
window.togMic = function (idx) {
    ...
    if (lessonMics[idx]) { try { lessonMics[idx].stop(); } catch (e) { } lessonMics[idx] = null; if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Gapirish'; return; }
    ...
    rec.onerror = e => {
        if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Gapirish'; lessonMics[idx] = null;
    ...
    rec.onend = () => { if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Gapirish'; if (st) st.innerHTML = '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Yozib olindi'; lessonMics[idx] = null; };
    try {
        rec.start(); lessonMics[idx] = rec;
        if (btn) btn.innerHTML = '⏹ To\'xtatish';
        if (st) st.innerHTML = '<i class="fa-solid fa-circle" style="color:#ef4444"></i> Yozmoqda...';
    } catch (e) {
        if (tr) tr.innerHTML = `<textarea style="width:100%;padding:8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#e8ecff;font-family:inherit" id="man${idx}" placeholder="Yozing..."></textarea>`;
    }
};
window.aiSpk = async function (idx, topic) {
    ...
    if (!text) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Avval gapiring!</span>'; return; }
    if (fb) fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> Baholayapti...';
...
window.markDone = function (idx) { lScore++; lTotal++; awardXP(10, 'speaking'); showToast('<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Bajarildi!', 'success'); };
...
window.selfChk = function (min) {
    ...
    const w = ta.value.trim() ? ta.value.trim().split(/\s+/).length : 0;
    if (w >= min) { fb.innerHTML = `<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${w} so'z!</span>`; lScore++; awardXP(15, 'writing'); }
    else { fb.innerHTML = `<span style="color:#f87171"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> ${min - w} so'z kam!</span>`; }
    lTotal++;
};
window.aiWrit = async function (title, words) {
    ...
    fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> Tekshirmoqda...';
    const r = await callAI(`Writing tekshirish. Mavzu: "${title}" (so'zlar: ${words}).\nMatn: "${ta.value.trim()}"\n${NATIVE_LANG} tilida (javob shu tilda bo'lsin): 1) Grammatika 2) Uslub 3) Tuzatilgan variant 4) IELTS bali: /9`, 800);
...
async function saveLessonCompletion(unitId, lessonKey, score, total, xpEarned, coinEarned) {
    ...
    try {
        ...
        if (allDone) {
            ...
            showToast(`<i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Unit to'liq! +50 XP +10 <i class="fa-solid fa-coins" style="color:#f5c842"></i> bonus!`, 'success');
        }
        await updateUserField(updates);
        UXP += xpEarned + (allDone ? 50 : 0);
        UCoin += coinEarned + (allDone ? 10 : 0);
        updateDisplays();
        showToast(`<i class="fa-solid fa-circle-check" style="color:#34d399"></i> +${xpEarned} XP +${coinEarned} <i class="fa-solid fa-coins" style="color:#f5c842"></i> saqlandi!`, 'success');
...
function showResult(lk, pct, xp, coin, unit, uid) {
    ...
    content.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="font-size:1.2rem;margin-bottom:20px">${pct >= 80 ? '<i class="fa-solid fa-trophy" style="color:#f5c842"></i> Mukammal!' : pct >= 60 ? '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Yaxshi!' : '<i class="fa-solid fa-dumbbell" style="color:#34d399"></i> Qayta urining!'}</div>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${nxt[lk] ? `<button onclick="window.openLesson('${uid}','${nxt[lk]}')" style="padding:12px;border-radius:12px;background:linear-gradient(135deg,#5b7cfa,#a78bfa);border:none;color:#fff;font-size:0.9rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-arrow-right"></i> Keyingi: ${lnames[nxt[lk]]}</button>` : `<div style="padding:14px;border-radius:12px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.25);color:#34d399;font-weight:700"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Unit to'liq bajarildi!</div>`}
        <button onclick="document.getElementById('unitModal').classList.remove('active');renderUnits()" style="padding:12px;border-radius:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:#e8ecff;cursor:pointer;font-size:0.9rem;font-family:inherit"><i class="fa-solid fa-house" style="color:#5b7cfa"></i> Unitlarga qaytish</button>
      </div>
    </div>`;
    showXPPop(`+${xp} XP`);
...
function openWModal(w) {
    ...
    c.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="display:flex;gap:8px;justify-content:center">
        <button onclick="window.spk('${w.e.replace(/'/g, "\\'")}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(91,124,250,0.15);border:1px solid rgba(91,124,250,0.3);color:#93c5fd;cursor:pointer;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i> Talaffuz</button>
        <button onclick="window.aiExWord('${w.e.replace(/'/g, "\\'")}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.25);color:#a78bfa;cursor:pointer;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI (1 token)</button>
      </div>
...
window.checkQuizOpt = function (btn, chosen, type) {
    ...
    else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: ${type === 'en2uz' ? curQuizWord.u : curQuizWord.e}</span>`; }
...
window.selectMatch2 = function (el) {
    ...
    if (!matchSel1) {
        ...
        if (matchSel1.dataset.id === el.dataset.id) {
            ...
            if (matchMatched.length === matchPairs.length) { const fb = $id('matchFeedback'); if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Barcha juftliklar!</span>'; }
        } else {
...
window.checkTyping = function () {
    ...
    if (val === w.e.toLowerCase()) {
        if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> To\'g\'ri!</span>';
        ...
        if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: ${w.e}</span>`;
...
window.checkGrammar = function (chosen, ans, exp) {
    ...
    if (chosen === ans) {
        if (fb) fb.innerHTML = `<div style="color:#34d399;padding:10px;border-radius:10px;background:rgba(52,211,153,0.1)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri! ${exp}</div>`;
        grammarScore2++; const el = $id('grammarScore'); if (el) el.textContent = grammarScore2; awardXP(12, 'grammar');
    } else {
        if (fb) fb.innerHTML = `<div style="color:#ef4444;padding:10px;border-radius:10px;background:rgba(239,68,68,0.1)"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Noto'g'ri. To'g'ri javob: <b>${ans}</b>. ${exp}</div>`;
    }
};
...
window.sendChatMessage = async function () {
    ...
    try {
        ...
        const tb = $id(typingId); if (tb) tb.remove();
        appendChat('assistant', `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> Xatolik: ${e.message || 'tarmoq muammosi'}`, false);
    } finally {
...
window.clearChatHistory = async function () {
    ...
    if (c) c.innerHTML = `<div class="chat-msg ai-msg"><div class="chat-avatar"><i class="fa-solid fa-robot"></i></div><div class="chat-bubble">Salom! Chat tarixi tozalandi. Yangi suhbatni boshlaylik! <i class="fa-solid fa-face-smile" style="color:#f5c842"></i></div></div>`;
