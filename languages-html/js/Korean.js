...
const RANKS = {
    none: { name: "Oddiy", icon: "<i class='fa-solid fa-square' style='color:#888'></i>", color: "#888", token_bonus: 0, xp_mult: 1, coin_mult: 1 },
    silver: { name: "Silver", icon: "<i class='fa-solid fa-medal' style='color:#C0C0C0'></i>", color: "#C0C0C0", token_bonus: 200, xp_mult: 1.3, coin_mult: 1.2 },
    gold: { name: "Gold", icon: "<i class='fa-solid fa-medal' style='color:#FFD700'></i>", color: "#FFD700", token_bonus: 500, xp_mult: 1.8, coin_mult: 1.5 },
    diamond: { name: "Diamond", icon: "<i class='fa-solid fa-gem' style='color:#00D4FF'></i>", color: "#B9F2FF", token_bonus: 1000, xp_mult: 2.5, coin_mult: 2 }
};
...
const UD_DATA = {
    beginner: [
        ...
        {
            id: 'kb1', emoji: '<i class="fa-solid fa-hand" style="color:#f5c842"></i>', title: '인사 (Greetings)', desc: "Salomlashish iboralari", level: 'beginner',
        ...
        {
            id: 'kb2', emoji: '<i class="fa-solid fa-hashtag" style="color:#60a5fa"></i>', title: '숫자 (Numbers)', desc: "Koreys raqamlari — sino va native", level: 'beginner',
        ...
        {
            id: 'kb3', emoji: '<i class="fa-solid fa-bowl-rice" style="color:#e5e7eb"></i>', title: '음식 (Food)', desc: "Koreys taomlari va ichimliklar", level: 'beginner',
        ...
        {
            id: 'kb4', emoji: '<i class="fa-solid fa-people-roof" style="color:#60a5fa"></i>', title: '가족 (Family)', desc: "Oila a\'zolarining nomlari", level: 'beginner',
        ...
        {
            id: 'kb5', emoji: '<i class="fa-solid fa-school" style="color:#60a5fa"></i>', title: '학교 (School)', desc: "Maktab hayoti va darslar", level: 'beginner',
    ...
    elementary: [
        {
            id: 'ke1', emoji: '<i class="fa-solid fa-city" style="color:#94a3b8"></i>', title: '도시 생활 (City Life)', desc: "Shahar hayoti va transport", level: 'elementary',
        ...
        {
            id: 'ke2', emoji: '<i class="fa-solid fa-briefcase" style="color:#94a3b8"></i>', title: '직업 (Jobs)', desc: "Kasb va ish hayoti", level: 'elementary',
        ...
        {
            id: 'ke3', emoji: '<i class="fa-solid fa-cart-shopping" style="color:#5b7cfa"></i>', title: '쇼핑 (Shopping)', desc: "Do\'kon va xarid qilish", level: 'elementary',
    ...
    'pre-intermediate': [
        {
            id: 'kp1', emoji: '<i class="fa-solid fa-newspaper" style="color:#94a3b8"></i>', title: '미래 계획 (Future Plans)', desc: "Kelajak zamoni va rejalar", level: 'pre-intermediate',
        ...
        {
            id: 'kp2', emoji: '<i class="fa-solid fa-globe" style="color:#5b7cfa"></i>', title: '문화 비교 (Culture)', desc: "Madaniyat va jamiyat", level: 'pre-intermediate',
    ...
    advanced: [
        {
            id: 'ka1', emoji: '<i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i>', title: '학술 글쓰기 (Academic)', desc: "Akademik yozuv uslubi", level: 'advanced',
        ...
        {
            id: 'ka2', emoji: '<i class="fa-solid fa-bullseye" style="color:#f87171"></i>', title: 'TOPIK 고급 (TOPIK Advanced)', desc: "TOPIK oliy daraja tayyorgarlik", level: 'advanced',
...
// ══════════════════════════════════════════════════════════════
const GRAMMAR_QS = [
    { q: "저는 학생___.", opts: ["이에요", "예요", "에요", "야"], ans: "이에요", exp: "'학생' undosh bilan tugaydi <i class='fa-solid fa-arrow-right'></i> 이에요" },
    { q: "어머니___ 선생님이에요.", opts: ["은", "는", "이", "가"], ans: "는", exp: "'어머니' unli bilan tugaydi <i class='fa-solid fa-arrow-right'></i> 는 (topic marker)" },
    { q: "밥___ 먹어요.", opts: ["이", "가", "을", "를"], ans: "을", exp: "'밥' undosh bilan tugaydi <i class='fa-solid fa-arrow-right'></i> 을 (object marker)" },
    ...
    { q: "저___ 한국 사람이에요.", opts: ["은", "는", "이", "가"], ans: "는", exp: "'저' unli bilan tugaydi <i class='fa-solid fa-arrow-right'></i> 는" },
...
function updateDisplays() {
    ...
    if (navUInfo) navUInfo.innerHTML = `<div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="font-size:0.78rem;color:#ef4444"><i class="fa-solid fa-star" style="color:#f5c842"></i> ${UXP.toLocaleString()}</span>
...
window.loadLBSection = async function (field, btn) {
    ...
    try {
        ...
        users.forEach((u, i) => {
            ...
            const displayName = esc((u.displayName || u.email || 'Foydalanuvchi')).slice(0, 18);
    ...
    content.innerHTML = `
      <div style="text-align:center;padding-bottom:20px">
        ...
        <div style="display:flex;gap:16px;justify-content:center;margin:16px 0;flex-wrap:wrap">
          ...
          <span style="color:#e74c3c"><i class="fa-solid fa-ticket" style="color:#a78bfa"></i> 2 token/dars</span>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px">
        ...
        return `<div onclick="window.openLesson('${unit.id}','${k}')" style="padding:16px;border-radius:12px;background:${done ? 'rgba(231,76,60,0.08)' : 'rgba(255,255,255,0.03)'};border:1px solid ${done ? 'rgba(231,76,60,0.3)' : 'rgba(255,255,255,0.08)'};cursor:pointer;transition:all 0.2s">
                ...
                ${done ? '<div style="font-size:0.72rem;color:#34d399;margin-top:4px"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Bajarildi</div>' : '<div style="font-size:0.72rem;color:#666;margin-top:4px">▶ Boshlash</div>'}
            </div>`;
    }).join('')}
      </div>
      <div>
        ...
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${unit.words.map(w => `<span onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.2);color:#ff6b6b;padding:4px 10px;border-radius:20px;font-size:0.76rem;cursor:pointer">${w} <i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></span>`).join('')}
        </div>
      </div>`;
    modal.classList.add('active');
...
function lessonA(unit) {
    ...
    const fills = words.slice(0, 4).map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px">
            <button onclick="window.chkFill(${i})" style="padding:6px 14px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.3);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> So'zlar va Romanizatsiya</h3>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(231,76,60,0.06);border:1px solid rgba(231,76,60,0.15);border-radius:12px">
      ...
      <div style="font-size:0.9rem;color:#ffb3b3;margin-bottom:8px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${unit.grammar_rule || ''}</div>
      <div style="font-size:0.85rem;color:#e74c3c;font-style:italic"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> ${unit.grammar_example || ''}</div>
      <button onclick="window.aiGrammarExplain('${unit.title}','${(unit.grammar_rule || '').replace(/'/g, "\\'")}')" style="margin-top:10px;padding:8px 16px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.25);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI batafsil (1 token)</button>
    ...
    <button onclick="window.finLessonA('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#e74c3c,#c0392b);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Grammatika darsini yakunlash</button>`;
}
...
function lessonB(unit) {
    ...
    <button onclick="window.finLessonB('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#22d3ee,#e74c3c);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Listening yakunlash</button>`;
}
...
function renderLex(exs, idx) {
    ...
    return `<div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      ...
      <div style="display:flex;gap:6px">
        ...
        ${idx + 1 < exs.length ? `<button onclick="window.nextLex(${idx + 1})" id="lexnxt${idx}" style="display:none;padding:7px 14px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.2);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-arrow-right"></i> Keyingi</button>` : ''}
      </div>
...
window.chkDict = function () {
    ...
    fb.innerHTML = `<div><strong>To'g'ri:</strong> ${dictSent}</div><div style="margin-top:6px"><strong>Siz:</strong> ${esc(inp.value)}</div><div style="font-size:0.9rem;font-weight:700;margin-top:6px;color:${pct >= 70 ? '#34d399' : '#ef4444'}">${pct}%</div>`;
...
function lessonC(unit) {
    ...
    <div style="margin-bottom:20px">
      ...
      <div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <button onclick="window.rdAloud()" style="margin-top:12px;padding:7px 14px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.2);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i> Koreyscha Tinglash</button>
      </div>
    </div>
    <div style="margin-bottom:20px">
    ...
    <div style="margin-bottom:20px">
      ...
      ${wh.map((w, i) => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap">
          ...
          <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:7px 12px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.2);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
...
function lessonD(unit) {
      ...
      ${topics.map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px;margin-bottom:8px">
            <button id="mbtn${i}" onclick="window.togMic(${i})" style="padding:8px 16px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.25);color:#e74c3c;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Gapirish</button>
          ...
          <div style="display:flex;gap:6px;margin-top:8px">
            <button onclick="window.aiSpk(${i},'${w}')" style="padding:6px 14px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.25);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI baholash (1 token)</button>
    ...
    <div style="margin-bottom:20px">
      ...
      <div style="padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px">
          <button onclick="window.aiWrit('${unit.title}')" style="padding:7px 14px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.25);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI tekshir (1 token)</button>
          <button onclick="window.selfChk(30)" style="padding:7px 14px;border-radius:8px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-chart-pie" style="color:#60a5fa"></i> So'z soni</button>
        </div>
    ...
    <div style="margin-bottom:20px">
      ...
      <div style="display:flex;gap:6px">
        ...
        <button onclick="window.spk('${woSent.replace(/'/g, "\\'")}',event)" style="padding:7px 14px;border-radius:8px;background:rgba(231,76,60,0.1);border:1px solid rgba(231,76,60,0.2);color:#e74c3c;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
      </div>
      <div id="wofb" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonD('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#e74c3c,#a78bfa);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Speaking & Writing yakunlash</button>`;
}
...
window.togMic = function (idx) {
    ...
    rec.onerror = e => { if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Gapirish'; lessonMics[idx] = null; if (e.error === 'not-allowed' && tr) tr.innerHTML = `<textarea style="width:100%;padding:8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#e8ecff;font-family:inherit" id="man${idx}" placeholder="한국어로 쓰세요..."></textarea>`; };
...
async function saveLessonCompletion(unitId, lessonKey, score, total, xpEarned, coinEarned) {
    ...
    if (!CU) { UXP += xpEarned; UCoin += coinEarned; updateDisplays(); renderUnits(); showToast(`<i class="fa-solid fa-circle-check" style="color:#34d399"></i> +${xpEarned} XP +${coinEarned} <i class="fa-solid fa-coins" style="color:#f5c842"></i>`, 'success'); return; }
    try {
        const updates = {
...
function openWModal(w) {
    ...
    c.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="display:flex;gap:8px;justify-content:center">
        <button onclick="window.spk('${w.e.replace(/'/g, "\\'")}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(231,76,60,0.15);border:1px solid rgba(231,76,60,0.3);color:#ff6b6b;cursor:pointer;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i> Tinglash</button>
      </div>
    </div>`;
...
window.checkQuizOpt = function (btn, chosen, type) {
    ...
    else { if (fb) fb.innerHTML = `<span style='color:#ef4444'><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: ${type === 'ko2uz' ? curQuizWord.u : curQuizWord.e}</span>`; }
...
window.clearChatHistory = async function () {
    ...
    if (c) c.innerHTML = `<div class="chat-msg ai-msg"><div class="chat-avatar"><i class="fa-solid fa-robot"></i></div><div class="chat-bubble">안녕하세요! Chat tarixi tozalandi. 한국어를 같이 배워요! <i class="fa-solid fa-face-smile" style="color:#f5c842"></i></div></div>`;
