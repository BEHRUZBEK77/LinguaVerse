...
const RANKS = {
    none: { name: "Обычный", icon: "<i class='fa-solid fa-square' style='color:#888'></i>", color: "#888", token_bonus: 0, xp_mult: 1, coin_mult: 1, price_coins: 0, xp_required: 0 },
    silver: { name: "Серебро", icon: "<i class='fa-solid fa-medal' style='color:#C0C0C0'></i>", color: "#C0C0C0", token_bonus: 200, xp_mult: 1.3, coin_mult: 1.2, price_coins: 500, xp_required: 0 },
    gold: { name: "Золото", icon: "<i class='fa-solid fa-medal' style='color:#FFD700'></i>", color: "#FFD700", token_bonus: 500, xp_mult: 1.8, coin_mult: 1.5, price_coins: 1500, xp_required: 500 },
    diamond: { name: "Алмаз", icon: "<i class='fa-solid fa-gem' style='color:#00D4FF'></i>", color: "#B9F2FF", token_bonus: 1000, xp_mult: 2.5, coin_mult: 2, price_coins: 4000, xp_required: 2000 }
};
...
const UD_DATA = {
    beginner: [
        {
            id: 'rb1', emoji: '<i class="fa-solid fa-hand" style="color:#f5c842"></i>', title: 'Привет! Знакомство', desc: 'Приветствие, прощание, вежливые слова', level: 'beginner',
        ...
        {
            id: 'rb2', emoji: '<i class="fa-solid fa-hashtag" style="color:#60a5fa"></i>', title: 'Числа 1–20', desc: 'Цифры, счёт, количество', level: 'beginner',
        ...
        {
            id: 'rb3', emoji: '<i class="fa-solid fa-palette" style="color:#a78bfa"></i>', title: 'Цвета и прилагательные', desc: 'Основные цвета и описания', level: 'beginner',
        ...
        {
            id: 'rb4', emoji: '<i class="fa-solid fa-people-roof" style="color:#60a5fa"></i>', title: 'Семья', desc: 'Члены семьи по-русски', level: 'beginner',
        ...
        {
            id: 'rb5', emoji: '<i class="fa-solid fa-apple-whole" style="color:#f87171"></i>', title: 'Еда и напитки', desc: 'Продукты, блюда, напитки', level: 'beginner',
    ...
    elementary: [
        {
            id: 're1', emoji: '<i class="fa-solid fa-house-chimney" style="color:#5b7cfa"></i>', title: 'Дом и комнаты', desc: 'Части дома, мебель', level: 'elementary',
        ...
        {
            id: 're2', emoji: '<i class="fa-solid fa-briefcase" style="color:#94a3b8"></i>', title: 'Профессии', desc: 'Самые популярные профессии', level: 'elementary',
        ...
        {
            id: 're3', emoji: '<i class="fa-solid fa-cart-shopping" style="color:#5b7cfa"></i>', title: 'Покупки', desc: 'В магазине, цены, торговля', level: 'elementary',
    ...
    'pre-intermediate': [
        {
            id: 'rp1', emoji: '<i class="fa-solid fa-hat-wizard" style="color:#a78bfa"></i>', title: 'Планы на будущее', desc: 'Будущее время, цели, намерения', level: 'pre-intermediate',
        ...
        {
            id: 'rp2', emoji: '<i class="fa-solid fa-bullseye" style="color:#f87171"></i>', title: 'Виды глаголов (НСВ/СВ)', desc: 'Несовершенный и совершенный вид', level: 'pre-intermediate',
    ...
    advanced: [
        {
            id: 'ra1', emoji: '<i class="fa-solid fa-pen" style="color:#60a5fa"></i>', title: 'Академическое письмо', desc: 'Эссе, отчёты, структура', level: 'advanced',
            words: ['более того', 'однако', 'поэтому', 'нюанс', 'красноречие', 'смягчать', 'беспрецедентный', 'педантичный', 'возможность', 'хотя'],
            xp: 200, coin: 90,
            grammar_rule: 'Средства связи: более того (кроме того), однако (но), поэтому (следовательно), несмотря на это (хотя). Структура: тезис <i class="fa-solid fa-arrow-right"></i> аргумент <i class="fa-solid fa-arrow-right"></i> вывод.',
        ...
        {
            id: 'ra2', emoji: '<i class="fa-solid fa-clipboard-list" style="color:#94a3b8"></i>', title: 'ТРКИ / ТОРФЛ подготовка', desc: 'Стратегии экзамена по русскому', level: 'advanced',
            words: ['более того', 'однако', 'поэтому', 'нюанс', 'красноречие', 'смягчать', 'беспрецедентный', 'педантичный', 'возможность', 'хотя'],
            xp: 250, coin: 110,
...
function checkTokenReset() {
    ...
    if (now - ULastReset >= resetMs) {
        ...
        showToast('<i class="fa-solid fa-bolt" style="color:#f5c842"></i> Токены обновлены!', 'success');
    }
}
...
function renderTokenBar() {
    ...
    el.innerHTML = `
        ...
        <div style="font-size:0.72rem;color:#888;margin-bottom:4px"><i class="fa-solid fa-ticket" style="color:#a78bfa"></i> Токены: <strong style="color:${color}">${UP === 'ultimate' ? '∞' : UTokens}</strong>/${UP === 'ultimate' ? '∞' : UMaxTokens}</div>
...
function showTokenEmptyModal(reason) {
    ...
    if (textEl) textEl.innerHTML = `<i class="fa-solid fa-ticket" style="color:#a78bfa"></i> Токены закончились!<br><span style="font-size:0.8rem;color:#666">${reason || 'Нужны токены для продолжения'}</span>`;
    if (timerEl) {
...
async function awardXP(base, skill) {
    ...
    if (coinFromXP > 0) { UCoin += coinFromXP; showToast(`<i class="fa-solid fa-coins" style="color:#f5c842"></i> +${coinFromXP} монет (бонус XP)`, 'info'); }
...
window.loadLBSection = async function (field, btn) {
    ...
    try {
        ...
        users.forEach((u, i) => {
            ...
            const initial = esc((u.displayName || u.email || 'П').charAt(0)).toUpperCase();
            html += `<div class="lb-row${isMe ? ' me' : ''}">
                <div class="lb-rank ${rankClass}">${rankIcon}</div>
                <div class="lb-avatar">${initial}</div>
                <div style="flex:1">
                    <div class="lb-name">${esc(u.displayName || u.email || 'Пользователь')}${isMe ? ' <span style="color:#c0392b;font-size:0.72rem">(вы)</span>' : ''}</div>
...
async function callAI(prompt, maxTok = 1000) {
    try {
        ...
        if (!r.ok) { return `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> Ошибка AI: ${r.status}.`; }
        const d = await r.json();
        return d.candidates?.[0]?.content?.parts?.[0]?.text || 'Ошибка получения ответа.';
    } catch (e) { return '<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> Ошибка подключения к AI.'; }
}
...
window.openUnit = function (unit) {
    ...
    const lnames = { A: "<i class='fa-solid fa-book-open' style='color:#a78bfa'></i> Грамматика и словарь", B: "<i class='fa-solid fa-headphones' style='color:#5b7cfa'></i> Аудирование", C: "<i class='fa-solid fa-book-open' style='color:#a78bfa'></i> Чтение", D: "<i class='fa-solid fa-microphone' style='color:#f87171'></i> Говорение и письмо" };
    ...
    content.innerHTML = `
      <div style="text-align:center;padding-bottom:20px">
        ...
        <div style="display:flex;gap:16px;justify-content:center;margin:16px 0;flex-wrap:wrap">
          ...
          <span style="color:#fbbf24"><i class="fa-solid fa-coins" style="color:#f5c842"></i> +${unit.coin} монет</span>
          <span style="color:#60a5fa"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> ${unit.words.length} слов</span>
          <span style="color:#c0392b"><i class="fa-solid fa-ticket" style="color:#a78bfa"></i> 2 токена/урок</span>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px">
        ...
        return `<div onclick="window.openLesson('${unit.id}','${k}')" style="padding:16px;border-radius:12px;background:${done ? 'rgba(52,211,153,0.08)' : 'rgba(255,255,255,0.03)'};border:1px solid ${done ? 'rgba(52,211,153,0.3)' : 'rgba(255,255,255,0.08)'};cursor:pointer;transition:all 0.2s" onmouseover="this.style.borderColor='${lcolors[k]}55'" onmouseout="this.style.borderColor='${done ? 'rgba(52,211,153,0.3)' : 'rgba(255,255,255,0.08)'}'">
                ...
                ${done ? `<div style="font-size:0.72rem;color:#34d399;margin-top:4px"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${sc}%</div>` : '<div style="font-size:0.72rem;color:#666;margin-top:4px">▶ Начать</div>'}
            </div>`;
    }).join('')}
      </div>
      <div>
        <div style="font-size:0.78rem;color:#666;margin-bottom:8px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> Слова:</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${unit.words.map(w => `<span onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.2);color:#e88;padding:4px 10px;border-radius:20px;font-size:0.76rem;cursor:pointer">${w} <i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></span>`).join('')}
        </div>
      </div>`;
    modal.classList.add('active');
...
function showLessonModal(unit, lk) {
    ...
    content.innerHTML = `<div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.08)">
        <button onclick="window.openUnit(window.__curUnit)" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#e8ecff;padding:6px 12px;border-radius:8px;cursor:pointer;font-size:0.8rem;font-family:inherit"><i class="fa-solid fa-arrow-left"></i> Назад</button>
...
function lessonA(unit) {
    ...
    const fills = words.slice(0, 4).map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px">
            <button onclick="window.chkFill(${i})" style="padding:6px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Проверить</button>
            <button onclick="window.aiExWord('${w.replace(/'/g, "\\'")}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.25);color:#e88;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI объяснит</button>
            <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(41,128,185,0.1);border:1px solid rgba(41,128,185,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
          </div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> Словарь урока</h3>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(192,57,43,0.06);border:1px solid rgba(192,57,43,0.15);border-radius:12px">
      <h3 style="margin-bottom:10px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> Правило грамматики</h3>
      <div style="font-size:0.9rem;color:#fdd;margin-bottom:8px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${unit.grammar_rule || ''}</div>
      <div style="font-size:0.85rem;color:#e88;font-style:italic"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> ${unit.grammar_example || ''}</div>
      <button onclick="window.aiGrammarExplain('${unit.title}','${(unit.grammar_rule || '').replace(/'/g, "\\'")}')" style="margin-top:10px;padding:8px 16px;border-radius:8px;background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.25);color:#e88;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI объяснит подробнее (1 токен)</button>
      <div id="gramRuleFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> Упражнения на заполнение</h3>
      ${fills}
      <div id="vocabAIFB" style="font-size:0.8rem"></div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-puzzle-piece" style="color:#a78bfa"></i> Упражнение на соответствие</h3>
    ...
    <button onclick="window.finLessonA('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#c0392b,#f39c12);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Завершить урок грамматики</button>`;
}
...
window.chkFill = function (i) {
    ...
    if (inp.value.trim().toLowerCase() === inp.dataset.ans.toLowerCase()) {
        fb.innerHTML = `<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Правильно! "${inp.dataset.ans}"</span>`;
        inp.style.borderColor = '#34d399'; lScore++; awardXP(10, 'grammar');
    } else {
        fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Правильный ответ: <strong>${inp.dataset.ans}</strong></span>`;
...
window.aiGrammarExplain = async function (title, rule) {
    ...
    const fb = $id('gramRuleFB'); if (fb) fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI анализирует...';
...
window.aiExWord = async function (word, e) {
    ...
    if (fb) fb.innerHTML = `<i class="fa-solid fa-robot" style="color:#a78bfa"></i> Анализирую слово "${word}"...`;
...
function lessonB(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-headphones" style="color:#5b7cfa"></i> Упражнение на аудирование</h3>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Диктант</h3>
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playDict('${unit.id}','slow')" style="padding:8px 16px;border-radius:8px;background:rgba(142,68,173,0.1);border:1px solid rgba(142,68,173,0.2);color:#a78bfa;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Медленно</button>
      </div>
      ...
      <div style="display:flex;gap:6px;margin-top:8px">
        <button onclick="window.chkDict()" style="padding:7px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Проверить</button>
        <button onclick="window.aiChkDict()" style="padding:7px 14px;border-radius:8px;background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.25);color:#e88;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI анализ (1 токен)</button>
      </div>
      <div id="dictFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonB('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#2980b9,#1abc9c);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Завершить аудирование</button>`;
}
...
function renderLex(exs, idx) {
    const ex = exs[idx];
    if (!ex) return '<div style="text-align:center;padding:20px;color:#34d399"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Все упражнения на аудирование выполнены!</div>';
    return `<div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playLex(${idx},'slow')" style="padding:8px 16px;border-radius:8px;background:rgba(142,68,173,0.1);border:1px solid rgba(142,68,173,0.2);color:#a78bfa;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Медленно</button>
      </div>
      ...
      <div style="display:flex;gap:6px">
        <button onclick="window.chkLex(${idx},${ex.c})" style="padding:7px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Проверить</button>
        ${idx + 1 < exs.length ? `<button onclick="window.nextLex(${idx + 1})" id="lexnxt${idx}" style="display:none;padding:7px 14px;border-radius:8px;background:rgba(41,128,185,0.1);border:1px solid rgba(41,128,185,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-arrow-right"></i> Следующий</button>` : ''}
      </div>
...
window.chkLex = function (idx, correct) {
    ...
    if (sel === undefined) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Выберите ответ!</span>'; return; }
    ...
    if (sel === correct) { if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Правильно!</span>'; lScore++; awardXP(15, 'listening'); }
    else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Правильный: <strong>${String.fromCharCode(65 + correct)}</strong></span>`; }
    lTotal++;
...
window.chkDict = function () {
    ...
    if (!inp || !fb || !dictSent) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Сначала послушайте аудио!</span>'; return; }
    ...
    fb.innerHTML = `<div><strong>Правильный текст:</strong> ${hl}</div><div style="margin-top:6px"><strong>Ваш текст:</strong> ${esc(inp.value)}</div><div style="font-size:0.9rem;font-weight:700;margin-top:6px;color:${pct >= 70 ? '#34d399' : '#ef4444'}">${pct}%</div>`;
...
window.aiChkDict = async function () {
    ...
    fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI анализирует...';
...
function lessonC(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-book-open" style="color:#a78bfa"></i> Чтение текста</h3>
      <div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px;margin-top:12px">
          <button onclick="window.rdAloud()" style="padding:7px 14px;border-radius:8px;background:rgba(41,128,185,0.1);border:1px solid rgba(41,128,185,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i> Слушать</button>
        </div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-circle-question" style="color:#93c5fd"></i> Вопросы по тексту</h3>
    ...
    <div style="margin-bottom:20px">
      ...
      ${wh.map((w, i) => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          ...
          <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:7px 12px;border-radius:8px;background:rgba(41,128,185,0.1);border:1px solid rgba(41,128,185,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
          <span id="whfb${i}" style="font-size:0.8rem;min-width:30px"></span>
        </div>`;
    }).join('')}
    </div>
    <button onclick="window.finLessonC('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#27ae60,#1abc9c);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Завершить чтение</button>`;
}
...
window.chkAllRQ = function (answers) {
    ...
    answers.forEach((correct, qi) => {
        ...
        if (sel === correct) { score++; if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Правильно!</span>'; }
        else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Правильный: ${String.fromCharCode(65 + correct)}</span>`; }
    });
...
function lessonD(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Упражнения на говорение</h3>
      ${topics.map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px;margin-bottom:8px">
            <button id="mbtn${i}" onclick="window.togMic(${i})" style="padding:8px 16px;border-radius:8px;background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.25);color:#e88;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Говорить</button>
            <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:8px 12px;border-radius:8px;background:rgba(41,128,185,0.1);border:1px solid rgba(41,128,185,0.2);color:#60a5fa;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
          </div>
          ...
          <div style="display:flex;gap:6px;margin-top:8px">
            <button onclick="window.aiSpk(${i},'${w}')" style="padding:6px 14px;border-radius:8px;background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.25);color:#e88;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI оценит (1 токен)</button>
            <button onclick="window.markDone(${i})" style="padding:6px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Выполнено</button>
          </div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Письменное упражнение</h3>
      <div style="padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px">
          <button onclick="window.aiWrit('${unit.title}','${unit.words.slice(0, 5).join(',')}')" style="padding:7px 14px;border-radius:8px;background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.25);color:#e88;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI проверит (1 токен)</button>
          <button onclick="window.selfChk(40)" style="padding:7px 14px;border-radius:8px;background:rgba(41,128,185,0.1);border:1px solid rgba(41,128,185,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-chart-pie" style="color:#60a5fa"></i> Подсчёт слов</button>
        </div>
        <div id="wfb" style="margin-top:8px;font-size:0.8rem"></div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#e8ecff;font-family:'Playfair Display',serif"><i class="fa-solid fa-shuffle" style="color:#5b7cfa"></i> Порядок слов</h3>
      ...
      <div style="display:flex;gap:6px">
        <button onclick="window.chkWO()" style="padding:7px 14px;border-radius:8px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.3);color:#34d399;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Проверить</button>
        <button onclick="window.rstWO()" style="padding:7px 14px;border-radius:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:#e8ecff;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Сбросить</button>
        <button onclick="window.spk('${woSent.replace(/'/g, "\\'")}',event)" style="padding:7px 14px;border-radius:8px;background:rgba(41,128,185,0.1);border:1px solid rgba(41,128,185,0.2);color:#60a5fa;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
      </div>
      <div id="wofb" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonD('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#8e44ad,#c0392b);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Завершить говорение и письмо</button>`;
}
...
window.chkWO = function () {
    const fb = $id('wofb');
    if (!woAns.length) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Расставьте слова!</span>'; return; }
    if (woAns.join(' ').toLowerCase() === (window.__woCorrect || '').toLowerCase()) {
        if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-trophy" style="color:#f5c842"></i> Отлично!</span>';
        awardXP(15, 'writing'); lScore++;
    } else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Правильно: <em>${window.__woCorrect}</em></span>`; }
    lTotal++;
};
window.togMic = function (idx) {
    ...
    if (lessonMics[idx]) { try { lessonMics[idx].stop(); } catch (e) { } lessonMics[idx] = null; if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Говорить'; return; }
    ...
    rec.onerror = e => {
        if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Говорить'; lessonMics[idx] = null;
    ...
    rec.onend = () => { if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Говорить'; if (st) st.innerHTML = '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Записано'; lessonMics[idx] = null; };
    try {
        rec.start(); lessonMics[idx] = rec;
        if (btn) btn.innerHTML = '⏹ Остановить';
        if (st) st.innerHTML = '<i class="fa-solid fa-circle" style="color:#ef4444"></i> Запись...';
    } catch (e) {
...
window.aiSpk = async function (idx, topic) {
    ...
    if (!text) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Сначала скажите что-нибудь!</span>'; return; }
    if (fb) fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI оценивает...';
    const r = await callAI(`Оценка говорения по-русски. Тема: "${topic}". Ученик сказал: "${text}".\nНа узбекском: 1) <i class="fa-solid fa-circle-check" style="color:#34d399"></i> Что хорошо 2) <i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Ошибки 3) <i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Исправленный вариант 4) <i class="fa-solid fa-star" style="color:#f5c842"></i> /10`, 700);
    if (fb) fb.innerHTML = esc(r).replace(/\n/g, '<br>');
    lScore++; lTotal++; awardXP(20, 'speaking');
};
window.markDone = function (idx) { lScore++; lTotal++; awardXP(10, 'speaking'); showToast('<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Выполнено!', 'success'); };
...
window.selfChk = function (min) {
    ...
    const w = ta.value.trim() ? ta.value.trim().split(/\s+/).length : 0;
    if (w >= min) { fb.innerHTML = `<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${w} слов!</span>`; lScore++; awardXP(15, 'writing'); }
    else { fb.innerHTML = `<span style="color:#f87171"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Не хватает ${min - w} слов!</span>`; }
    lTotal++;
};
window.aiWrit = async function (title, words) {
    ...
    fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> Проверяю...';
...
async function saveLessonCompletion(unitId, lessonKey, score, total, xpEarned, coinEarned) {
    ...
    try {
        ...
        if (allDone) {
            ...
            showToast(`<i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Юнит пройден! +50 XP +10 <i class="fa-solid fa-coins" style="color:#f5c842"></i> бонус!`, 'success');
        }
        await updateUserField(updates);
        UXP += xpEarned + (allDone ? 50 : 0);
        UCoin += coinEarned + (allDone ? 10 : 0);
        updateDisplays();
        showToast(`<i class="fa-solid fa-circle-check" style="color:#34d399"></i> +${xpEarned} XP +${coinEarned} <i class="fa-solid fa-coins" style="color:#f5c842"></i> сохранено!`, 'success');
...
function showResult(lk, pct, xp, coin, unit, uid) {
    ...
    content.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="font-size:1.2rem;margin-bottom:20px">${pct >= 80 ? '<i class="fa-solid fa-trophy" style="color:#f5c842"></i> Отлично!' : pct >= 60 ? '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Хорошо!' : '<i class="fa-solid fa-dumbbell" style="color:#34d399"></i> Попробуйте ещё раз!'}</div>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${nxt[lk] ? `<button onclick="window.openLesson('${uid}','${nxt[lk]}')" style="padding:12px;border-radius:12px;background:linear-gradient(135deg,#c0392b,#f39c12);border:none;color:#fff;font-size:0.9rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-arrow-right"></i> Следующий: ${lnames[nxt[lk]]}</button>` : `<div style="padding:14px;border-radius:12px;background:rgba(52,211,153,0.1);border:1px solid rgba(52,211,153,0.25);color:#34d399;font-weight:700"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Юнит полностью пройден!</div>`}
...
function openWModal(w) {
    ...
    c.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="display:flex;gap:8px;justify-content:center">
        <button onclick="window.spk('${w.e.replace(/'/g, "\\'")}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(41,128,185,0.15);border:1px solid rgba(41,128,185,0.3);color:#60a5fa;cursor:pointer;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i> Произношение</button>
        <button onclick="window.aiExWord('${w.e.replace(/'/g, "\\'")}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(192,57,43,0.1);border:1px solid rgba(192,57,43,0.25);color:#e88;cursor:pointer;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI объяснит (1 токен)</button>
      </div>
...
window.checkQuizOpt = function (btn, chosen, type) {
    ...
    else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Правильно: ${type === 'ru2uz' ? curQuizWord.u : curQuizWord.e}</span>`; }
...
window.selectMatch2 = function (el) {
    ...
    if (!matchSel1) {
        ...
        if (matchSel1.dataset.id === el.dataset.id) {
            ...
            if (matchMatched.length === matchPairs.length) { const fb = $id('matchFeedback'); if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Все пары найдены!</span>'; }
        } else {
...
window.checkTyping = function () {
    ...
    if (val === w.e.toLowerCase()) {
        if (fb) fb.innerHTML = '<span style="color:#34d399"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Правильно!</span>';
        ...
        if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Правильно: ${w.e}</span>`;
...
window.checkGrammar = function (chosen, ans, exp) {
    ...
    if (chosen === ans) {
        if (fb) fb.innerHTML = `<div style="color:#34d399;padding:10px;border-radius:10px;background:rgba(52,211,153,0.1)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Правильно! ${exp}</div>`;
        grammarScore2++; const el = $id('grammarScore'); if (el) el.textContent = grammarScore2; awardXP(12, 'grammar');
    } else {
        if (fb) fb.innerHTML = `<div style="color:#ef4444;padding:10px;border-radius:10px;background:rgba(239,68,68,0.1)"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Неправильно. Правильный ответ: <b>${ans}</b>. ${exp}</div>`;
    }
};
...
window.sendChatMessage = async function () {
    ...
    try {
        ...
        if (!resp.ok) { appendChat('assistant', `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> Ошибка AI: ${resp.status}`, false); return; }
        ...
        const tb = $id(typingId); if (tb) tb.remove();
        appendChat('assistant', `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> Ошибка: ${e.message || 'сетевая проблема'}`, false);
    } finally {
...
window.clearChatHistory = async function () {
    ...
    if (c) c.innerHTML = `<div class="chat-msg ai-msg"><div class="chat-avatar"><i class="fa-solid fa-robot"></i></div><div class="chat-bubble">Привет! История очищена. Начнём сначала! <i class="fa-solid fa-face-smile" style="color:#f5c842"></i></div></div>`;
