// XSS himoyasi: Firestore'dan kelgan foydalanuvchi matnini (displayName, email
// va h.k.) innerHTML ichiga qo'yishdan oldin ekranlaymiz.
function esc(s) {
...
// ═══ REJALAR — nomlar va narxlar js/lv-core.js bilan MOS (yagona manba) ═══
// Kunlik AI limitlarini lv-ai-bridge.js boshqaradi; bu yerdagi token_bonus
// faqat sahifa ichidagi mashqlar uchun ichki "yoqilg'i".
const PLANS = {
    free:    { name: "Bepul",   icon: "<i class='fa-solid fa-seedling' style='color:#34d399'></i>", price_uzs: 0,     token_bonus: 2000,  token_reset_mult: 1, ai_quality: "standard", xp_mult: 1,   coin_mult: 1,   features: ["NOVA AI — kuniga 40 savol", "Speaking 15 daqiqa/kun", "Kuniga 4 AI dars", "Coin va XP tizimi"] },
    nova:    { name: "NOVA",    icon: "<i class='fa-solid fa-bolt' style='color:#f5c842'></i>", price_uzs: 39999, token_bonus: 6000,  token_reset_mult: 2, ai_quality: "enhanced", xp_mult: 1.5, coin_mult: 1.3, features: ["NOVA 200 + PRO 50 savol/kun", "PRO — o'ylash rejimi", "Speaking 40 daqiqa/kun", "12 AI dars/kun"] },
    mega:    { name: "MEGA",    icon: "<i class='fa-solid fa-rocket' style='color:#a78bfa'></i>", price_uzs: 49999, token_bonus: 12000, token_reset_mult: 3, ai_quality: "advanced", xp_mult: 2,   coin_mult: 1.6, features: ["MEGA 150 + PRO+ 60 savol/kun", "Quyi modellar 2x limitda", "Speaking 80 daqiqa/kun", "30 AI dars/kun"] },
    premium: { name: "PREMIUM", icon: "<i class='fa-solid fa-crown' style='color:#f5c842'></i>", price_uzs: 79999, token_bonus: 25000, token_reset_mult: 4, ai_quality: "ultimate", xp_mult: 2.5, coin_mult: 2,   features: ["PREMIUM 220 + MEGA+ 90 savol/kun", "Barchasi maksimal limitda", "Speaking 150 daqiqa/kun", "Imtihonga tayyorlov"] }
};
// Eski reja nomlari bazada uchrab qolsa ham ishlashi uchun taxalluslar
PLANS.pro = PLANS.nova; PLANS.basic = PLANS.nova; PLANS.starter = PLANS.nova;
PLANS.ultimate = PLANS.premium; PLANS.vip = PLANS.premium;

const RANKS = {
    none: { name: "Oddiy", icon: "<i class='fa-solid fa-square' style='color:#888'></i>", color: "#888", token_bonus: 0, xp_mult: 1, coin_mult: 1, price_coins: 0, xp_required: 0, badge_color: "#555", perks: [] },
    silver: { name: "Silver", icon: "<i class='fa-solid fa-medal' style='color:#C0C0C0'></i>", color: "#C0C0C0", token_bonus: 200, xp_mult: 1.3, coin_mult: 1.2, price_coins: 500, xp_required: 0, badge_color: "#A8A8A8", perks: ["+200 coin har darsda", "+30% XP", "+20% Coin"] },
    gold: { name: "Gold", icon: "<i class='fa-solid fa-medal' style='color:#FFD700'></i>", color: "#FFD700", token_bonus: 500, xp_mult: 1.8, coin_mult: 1.5, price_coins: 1500, xp_required: 500, badge_color: "#E6B800", perks: ["+500 coin har darsda", "+80% XP", "+50% Coin"] },
    diamond: { name: "Diamond", icon: "<i class='fa-solid fa-gem' style='color:#00D4FF'></i>", color: "#B9F2FF", token_bonus: 1000, xp_mult: 2.5, coin_mult: 2, price_coins: 4000, xp_required: 2000, badge_color: "#00D4FF", perks: ["+1000 coin har darsda", "+150% XP", "+100% Coin", "Leaderboard badge"] }
};

const PLAN_LIMITS = { free: 10, nova: 40, mega: 100, premium: 250, pro: 40, basic: 40, starter: 40, ultimate: 250, vip: 250 };
const PLAN_LABELS = { free: "Bepul", nova: "NOVA", mega: "MEGA", premium: "PREMIUM", pro: "NOVA", basic: "NOVA", starter: "NOVA", ultimate: "PREMIUM", vip: "PREMIUM" };
const PLAN_COLORS = { free: "#94a3b8", nova: "#3b82f6", mega: "#8b5cf6", premium: "#f59e0b", pro: "#3b82f6", basic: "#3b82f6", starter: "#3b82f6", ultimate: "#f59e0b", vip: "#f59e0b" };
...
const UD_DATA = {
    principiante: [
        {
            id: 'p0', emoji: '<i class="fa-solid fa-font" style="color:#60a5fa"></i>', title: 'El Alfabeto', desc: "Ispan alifbosini o'rganish", level: 'principiante',
        ...
        {
            id: 'p1', emoji: '<i class="fa-solid fa-hand" style="color:#f5c842"></i>', title: 'Saludos', desc: "Salomlashish va tanishish", level: 'principiante',
        ...
        {
            id: 'p2', emoji: '<i class="fa-solid fa-hashtag" style="color:#60a5fa"></i>', title: 'Los Números', desc: "Raqamlar va sanash", level: 'principiante',
        ...
        {
            id: 'p3', emoji: '<i class="fa-solid fa-palette" style="color:#a78bfa"></i>', title: 'Los Colores', desc: "Ispan tilida ranglar", level: 'principiante',
        ...
        {
            id: 'p4', emoji: '<i class="fa-solid fa-people-roof" style="color:#60a5fa"></i>', title: 'La Familia', desc: "Oila a'zolari", level: 'principiante',
        ...
        {
            id: 'p5', emoji: '<i class="fa-solid fa-apple-whole" style="color:#f87171"></i>', title: 'Comida y Bebida', desc: "Taom va ichimliklar", level: 'principiante',
    ...
    elemental: [
        {
            id: 'e1', emoji: '<i class="fa-solid fa-house-chimney" style="color:#5b7cfa"></i>', title: 'La Casa', desc: "Uy va xonalar", level: 'elemental',
        ...
        {
            id: 'e2', emoji: '<i class="fa-solid fa-briefcase" style="color:#94a3b8"></i>', title: 'Las Profesiones', desc: "Kasblar", level: 'elemental',
        ...
        {
            id: 'e3', emoji: '<i class="fa-solid fa-cart-shopping" style="color:#5b7cfa"></i>', title: 'De Compras', desc: "Xarid qilish", level: 'elemental',
    ...
    intermedio: [
        {
            id: 'i1', emoji: '<i class="fa-solid fa-hat-wizard" style="color:#a78bfa"></i>', title: 'Planes Futuros', desc: "Kelajak zamonlari", level: 'intermedio',
        ...
        {
            id: 'i2', emoji: '<i class="fa-solid fa-bullseye" style="color:#f87171"></i>', title: 'El Subjuntivo', desc: "Subjuntivo zamonining asoslari", level: 'intermedio',
            words: ['sin embargo', 'aunque', 'por lo tanto', 'además', 'oportunidad', 'matiz', 'elocuencia', 'mitigar', 'sin precedentes', 'meticuloso'],
            xp: 140, coin: 65,
    ...
    avanzado: [
        {
            id: 'a1', emoji: '<i class="fa-solid fa-pen" style="color:#60a5fa"></i>', title: 'Escritura Académica', desc: "Akademik yozuv", level: 'avanzado',
        ...
        {
            id: 'a2', emoji: '<i class="fa-solid fa-clipboard-list" style="color:#94a3b8"></i>', title: 'DELE / SIELE', desc: "DELE/SIELE imtihon tayyorligi", level: 'avanzado',
            words: ['además', 'sin embargo', 'por lo tanto', 'matiz', 'elocuencia', 'mitigar', 'sin precedentes', 'meticuloso', 'oportunidad', 'aunque'],
            xp: 250, coin: 110,
...
// ══════════════════════════════════════════════════════════════
const GRAMMAR_QS = [
    { q: "Ella ___ a la escuela cada día.", opts: ["van", "vas", "va", "vamos"], ans: "va", exp: "3-shaxs birlik: ir <i class='fa-solid fa-arrow-right'></i> va" },
...
function checkTokenReset() {
    ...
    if (now - ULastReset >= resetMs) {
        ...
        showToast('<i class="fa-solid fa-bolt" style="color:#f5c842"></i> Tokenlar yangilandi!', 'success');
    }
}
...
function renderTokenBar() {
    ...
    el.innerHTML = `
        ...
        <div style="font-size:0.72rem;color:#888;margin-bottom:4px"><i class="fa-solid fa-ticket" style="color:#a78bfa"></i> Token: <strong style="color:${color}">${UP === 'ultimate' ? '∞' : UTokens}</strong>/${UP === 'ultimate' ? '∞' : UMaxTokens}</div>
...
function showTokenEmptyModal(reason) {
    ...
    if (textEl) textEl.innerHTML = `<i class="fa-solid fa-ticket" style="color:#a78bfa"></i> Tokenlar tugadi!<br><span style="font-size:0.8rem;color:#666">${reason || 'Davom etish uchun token kerak'}</span>`;
    if (timerEl) {
...
async function awardXP(base, skill) {
    ...
    if (coinFromXP > 0) { UCoin += coinFromXP; showToast(`<i class="fa-solid fa-coins" style="color:#f5c842"></i> +${coinFromXP} coin (XP bonus)`, 'info'); }
...
function updateDisplays() {
    ...
    if (navUInfo) navUInfo.innerHTML = `<div style="display:flex;align-items:center;gap:8px">
        ...
        <span style="font-size:0.78rem;color:#f0a500"><i class="fa-solid fa-star" style="color:#f5c842"></i> ${UXP.toLocaleString()}</span>
        <span style="font-size:0.78rem;color:#f0a500"><i class="fa-solid fa-coins" style="color:#f5c842"></i> ${UCoin.toLocaleString()}</span>
    </div>`;
}
...
window.loadLBSection = async function (field, btn) {
    ...
    try {
        const q = query(collection(_db, 'leaderboard'), orderBy(field, 'desc'), limit(20));
        ...
        users.forEach((u, i) => {
            ...
            const initial = esc((u.displayName || u.email || 'U').charAt(0)).toUpperCase();
            html += `<div class="lb-row${isMe ? ' me' : ''}">
                <div class="lb-rank ${rankClass}">${rankIcon}</div>
                <div class="lb-avatar">${initial}</div>
                <div style="flex:1">
                    <div class="lb-name">${esc(u.displayName || u.email || 'Foydalanuvchi')}${isMe ? ' <span style="color:#d4456a;font-size:0.72rem">(siz)</span>' : ''}</div>
...
async function callAI(prompt, maxTok = 1000) {
    try {
        ...
        if (!r.ok) { return `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> AI xatolik: ${r.status}.`; }
        const d = await r.json();
        return d.candidates?.[0]?.content?.parts?.[0]?.text || 'Javob olishda xatolik.';
    } catch (e) { return '<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> AI bilan bog\'lanishda xatolik.'; }
}
    ...
    units.forEach((unit, i) => {
        ...
        card.innerHTML = `
          ...
          <div style="font-size:0.76rem;display:flex;gap:10px">
            ...
            <span style="color:#f0a500">+${unit.coin} <i class="fa-solid fa-coins" style="color:#f5c842"></i></span>
            ${isComp ? '<span style="color:#2ecc71"><i class="fa-solid fa-circle-check" style="color:#34d399"></i></span>' : ''}
          </div>`;
...
window.openUnit = function (unit) {
    ...
    const lnames = { A: "<i class='fa-solid fa-book-open' style='color:#a78bfa'></i> Gramática & Vocabulario", B: "<i class='fa-solid fa-headphones' style='color:#5b7cfa'></i> Comprensión Auditiva", C: "<i class='fa-solid fa-book-open' style='color:#a78bfa'></i> Lectura", D: "<i class='fa-solid fa-microphone' style='color:#f87171'></i> Expresión Oral & Escrita" };
    ...
    content.innerHTML = `
      <div style="text-align:center;padding-bottom:20px">
        ...
        <div style="display:flex;gap:16px;justify-content:center;margin:16px 0;flex-wrap:wrap">
          <span style="color:#f0a500"><i class="fa-solid fa-star" style="color:#f5c842"></i> +${unit.xp} XP</span>
          <span style="color:#f0a500"><i class="fa-solid fa-coins" style="color:#f5c842"></i> +${unit.coin} Moneda</span>
          <span style="color:#60a5fa"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> ${unit.words.length} palabras</span>
          <span style="color:#d4456a"><i class="fa-solid fa-ticket" style="color:#a78bfa"></i> 2 token/lección</span>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px">
        ...
        return `<div onclick="window.openLesson('${unit.id}','${k}')" style="padding:16px;border-radius:12px;background:${done ? 'rgba(46,204,113,0.08)' : 'rgba(255,255,255,0.03)'};border:1px solid ${done ? 'rgba(46,204,113,0.3)' : 'rgba(255,255,255,0.08)'};cursor:pointer;transition:all 0.2s" onmouseover="this.style.borderColor='${lcolors[k]}55'" onmouseout="this.style.borderColor='${done ? 'rgba(46,204,113,0.3)' : 'rgba(255,255,255,0.08)'}'">
                ...
                ${done ? `<div style="font-size:0.72rem;color:#2ecc71;margin-top:4px"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${sc}%</div>` : '<div style="font-size:0.72rem;color:#666;margin-top:4px">▶ Comenzar</div>'}
            </div>`;
    }).join('')}
      </div>
      <div>
        <div style="font-size:0.78rem;color:#666;margin-bottom:8px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> Palabras:</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${unit.words.map(w => `<span onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.2);color:#f0a500;padding:4px 10px;border-radius:20px;font-size:0.76rem;cursor:pointer">${w} <i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></span>`).join('')}
        </div>
      </div>`;
    modal.classList.add('active');
...
function showLessonModal(unit, lk) {
    ...
    content.innerHTML = `<div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.08)">
        <button onclick="window.openUnit(window.__curUnit)" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#fff5f0;padding:6px 12px;border-radius:8px;cursor:pointer;font-size:0.8rem;font-family:inherit"><i class="fa-solid fa-arrow-left"></i> Volver</button>
...
function lessonA(unit) {
    ...
    const fills = words.slice(0, 4).map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px">
            <button onclick="window.chkFill(${i})" style="padding:6px 14px;border-radius:8px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);color:#2ecc71;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Comprobar</button>
            <button onclick="window.aiExWord('${w.replace(/'/g, "\\'")}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.25);color:#f0a500;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI</button>
            <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:6px 14px;border-radius:8px;background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.2);color:#d4456a;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
          </div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-book-open-reader" style="color:#a78bfa"></i> Vocabulario</h3>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:10px">
        ...
        return `<div style="padding:14px;border-radius:12px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06)">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
                ...
                <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="background:none;border:none;cursor:pointer;font-size:1rem"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
              </div>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(212,69,106,0.06);border:1px solid rgba(212,69,106,0.15);border-radius:12px">
      <h3 style="margin-bottom:10px;color:#fff5f0"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> Regla Gramatical</h3>
      <div style="font-size:0.9rem;color:#ffd4de;margin-bottom:8px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${unit.grammar_rule || ''}</div>
      <div style="font-size:0.85rem;color:#f0a500;font-style:italic"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> ${unit.grammar_example || ''}</div>
      <button onclick="window.aiGrammarExplain('${unit.title}','${(unit.grammar_rule || '').replace(/'/g, "\\'")}')" style="margin-top:10px;padding:8px 16px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.25);color:#f0a500;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-robot' style='color:#a78bfa'></i> AI batafsil tushuntirsin (1 token)</button>
      <div id="gramRuleFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> Ejercicios de Relleno</h3>
      ${fills}
      <div id="vocabAIFB" style="font-size:0.8rem"></div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-puzzle-piece" style="color:#a78bfa"></i> Ejercicio de Emparejamiento</h3>
    ...
    <button onclick="window.finLessonA('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#d4456a,#f0a500);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Gramática leccioni yakunlash</button>`;
}
...
window.chkFill = function (i) {
    ...
    if (inp.value.trim().toLowerCase() === inp.dataset.ans.toLowerCase()) {
        fb.innerHTML = `<span style="color:#2ecc71"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ¡Correcto! "${inp.dataset.ans}"</span>`;
        inp.style.borderColor = '#2ecc71'; lScore++; awardXP(10, 'grammar');
    } else {
        fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Respuesta correcta: <strong>${inp.dataset.ans}</strong></span>`;
...
window.selMatch = function (el, type, val) {
    ...
    if (mSel.e !== null && mSel.u !== null) {
        ...
        if (wd && wd.u === mSel.u) {
            ...
            awardXP(5, 'grammar'); showToast(`<i class="fa-solid fa-circle-check" style="color:#34d399"></i> "${w}" = "${mSel.u}"!`, 'success');
        } else {
...
window.aiGrammarExplain = async function (title, rule) {
    ...
    const fb = $id('gramRuleFB'); if (fb) fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI tahlil qilmoqda...';
    const r = await callAI(`"${title}" mavzusida "${rule}" ispan tili grammatika qoidasini ${NATIVE_LANG} tilida (javob shu tilda bo'lsin) tushuntir. 3 ta misol keltir.`, 800);
    if (fb) fb.innerHTML = esc(r).replace(/\n/g, '<br>');
};
...
window.aiExWord = async function (word, e) {
    ...
    if (fb) fb.innerHTML = `<i class="fa-solid fa-robot" style="color:#a78bfa"></i> "${word}" tahlil qilmoqda...`;
    const r = await callAI(`"${word}" ispancha so'zini ${NATIVE_LANG} tilida (javob shu tilda bo'lsin): 1) Ma'nosi 2) 3 misol 3) Eslatma`, 600);
    if (fb) fb.innerHTML = esc(r).replace(/\n/g, '<br>');
};
...
function lessonB(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-headphones" style="color:#5b7cfa"></i> Comprensión Auditiva</h3>
    ...
    <div style="margin-bottom:20px;padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Dictado</h3>
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playDict('${unit.id}','slow')" style="padding:8px 16px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.2);color:#f0a500;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Lento</button>
      </div>
      ...
      <div style="display:flex;gap:6px;margin-top:8px">
        <button onclick="window.chkDict()" style="padding:7px 14px;border-radius:8px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);color:#2ecc71;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Comprobar</button>
        <button onclick="window.aiChkDict()" style="padding:7px 14px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.25);color:#f0a500;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI tahlil (1 token)</button>
      </div>
      <div id="dictFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonB('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#22d3ee,#d4456a);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Comprensión auditiva yakunlash</button>`;
}
...
function renderLex(exs, idx) {
    const ex = exs[idx];
    if (!ex) return '<div style="text-align:center;padding:20px;color:#2ecc71"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> ¡Todos los ejercicios completados!</div>';
    return `<div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <button onclick="window.playLex(${idx},'slow')" style="padding:8px 16px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.2);color:#f0a500;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Lento</button>
      </div>
      ...
      <div style="font-size:0.75rem;color:#666;margin-bottom:10px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${ex.tip}</div>
      <div style="display:flex;gap:6px">
        <button onclick="window.chkLex(${idx},${ex.c})" style="padding:7px 14px;border-radius:8px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);color:#2ecc71;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Comprobar</button>
        ${idx + 1 < exs.length ? `<button onclick="window.nextLex(${idx + 1})" id="lexnxt${idx}" style="display:none;padding:7px 14px;border-radius:8px;background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.2);color:#d4456a;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-arrow-right"></i> Siguiente</button>` : ''}
      </div>
...
window.chkLex = function (idx, correct) {
    ...
    if (sel === undefined) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Selecciona una respuesta!</span>'; return; }
    ...
    if (sel === correct) { if (fb) fb.innerHTML = '<span style="color:#2ecc71"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ¡Correcto!</span>'; lScore++; awardXP(15, 'listening'); }
    else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Correcto: <strong>${String.fromCharCode(65 + correct)}</strong></span>`; }
    lTotal++;
...
window.chkDict = function () {
    ...
    if (!inp || !fb || !dictSent) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> ¡Primero escucha el audio!</span>'; return; }
    ...
    fb.innerHTML = `<div><strong>Correcto:</strong> ${hl}</div><div style="margin-top:6px"><strong>Tú:</strong> ${esc(inp.value)}</div><div style="font-size:0.9rem;font-weight:700;margin-top:6px;color:${pct >= 70 ? '#2ecc71' : '#ef4444'}">${pct}%</div>`;
...
window.aiChkDict = async function () {
    ...
    fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI tahlil qilmoqda...';
    const r = await callAI(`Dictado tahlili ${NATIVE_LANG} tilida (javob shu tilda bo'lsin):\nAsl: "${dictSent}"\nO'quvchi: "${inp.value.trim()}"\n1) Xatolari 2) Ball: /10 3) Maslahat`, 600);
    fb.innerHTML = esc(r).replace(/\n/g, '<br>');
};
...
function lessonC(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-book-open" style="color:#a78bfa"></i> Lectura</h3>
      <div style="padding:16px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px;margin-top:12px">
          <button onclick="window.rdAloud()" style="padding:7px 14px;border-radius:8px;background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.2);color:#d4456a;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i> Escuchar</button>
        </div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-circle-question" style="color:#93c5fd"></i> Preguntas de Comprensión</h3>
      ...
      <button onclick="window.chkAllRQ(${JSON.stringify(qs.map(q => q.c))})" style="padding:8px 16px;border-radius:8px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);color:#2ecc71;cursor:pointer;font-size:0.82rem;font-family:inherit;margin-top:8px"><i class="fa-solid fa-check" style="color:#34d399"></i> Comprobar todo</button>
      <div id="rdTotFB" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-font" style="color:#60a5fa"></i> Escritura de palabras</h3>
      ${wh.map((w, i) => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          ...
          <button onclick="window.chkWH(${i})" style="padding:7px 12px;border-radius:8px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);color:#2ecc71;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i></button>
          <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:7px 12px;border-radius:8px;background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.2);color:#d4456a;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
          <span id="whfb${i}" style="font-size:0.8rem;min-width:30px"></span>
        </div>`;
    }).join('')}
    </div>
    <button onclick="window.finLessonC('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#2ecc71,#22d3ee);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Lectura yakunlash</button>`;
}
...
window.chkAllRQ = function (answers) {
    ...
    answers.forEach((correct, qi) => {
        ...
        if (sel === correct) { score++; if (fb) fb.innerHTML = '<span style="color:#2ecc71"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ¡Correcto!</span>'; }
        else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Correcto: ${String.fromCharCode(65 + correct)}</span>`; }
    });
...
window.chkWH = function (i) {
    ...
    if (inp.value.trim().toLowerCase() === inp.dataset.ans.toLowerCase()) {
        fb.innerHTML = '<i class="fa-solid fa-circle-check" style="color:#34d399"></i>'; inp.style.borderColor = '#2ecc71'; awardXP(8, 'writing');
    } else { fb.innerHTML = '<i class="fa-solid fa-circle-xmark" style="color:#f87171"></i>'; inp.style.borderColor = '#ef4444'; }
};
...
function lessonD(unit) {
    ...
    return `
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Expresión Oral</h3>
      ${topics.map((w, i) => {
        ...
        return `<div style="margin-bottom:14px;padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
          ...
          <div style="display:flex;gap:6px;margin-bottom:8px">
            <button id="mbtn${i}" onclick="window.togMic(${i})" style="padding:8px 16px;border-radius:8px;background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.25);color:#d4456a;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class="fa-solid fa-microphone" style="color:#f87171"></i> Hablar</button>
            <button onclick="window.spk('${w.replace(/'/g, "\\'")}',event)" style="padding:8px 12px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.2);color:#f0a500;cursor:pointer;font-size:0.82rem;font-family:inherit"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
          </div>
          ...
          <div style="display:flex;gap:6px;margin-top:8px">
            <button onclick="window.aiSpk(${i},'${w}')" style="padding:6px 14px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.25);color:#f0a500;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI baholash (1 token)</button>
            <button onclick="window.markDone(${i})" style="padding:6px 14px;border-radius:8px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);color:#2ecc71;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Completado</button>
          </div>
    ...
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-pen-nib" style="color:#60a5fa"></i> Expresión Escrita</h3>
      <div style="padding:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div style="display:flex;gap:6px">
          <button onclick="window.aiWrit('${unit.title}','${unit.words.slice(0, 5).join(',')}')" style="padding:7px 14px;border-radius:8px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.25);color:#f0a500;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI (1 token)</button>
          <button onclick="window.selfChk(40)" style="padding:7px 14px;border-radius:8px;background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.2);color:#d4456a;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-chart-pie" style="color:#60a5fa"></i> Contar palabras</button>
        </div>
        <div id="wfb" style="margin-top:8px;font-size:0.8rem"></div>
      </div>
    </div>
    <div style="margin-bottom:20px">
      <h3 style="margin-bottom:12px;color:#fff5f0"><i class="fa-solid fa-shuffle" style="color:#5b7cfa"></i> Ordenar Palabras</h3>
      ...
      <div style="display:flex;gap:6px">
        <button onclick="window.chkWO()" style="padding:7px 14px;border-radius:8px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);color:#2ecc71;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-check" style="color:#34d399"></i> Comprobar</button>
        <button onclick="window.rstWO()" style="padding:7px 14px;border-radius:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:#fff5f0;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Reiniciar</button>
        <button onclick="window.spk('${woSent.replace(/'/g, "\\'")}',event)" style="padding:7px 14px;border-radius:8px;background:rgba(212,69,106,0.1);border:1px solid rgba(212,69,106,0.2);color:#d4456a;cursor:pointer;font-size:0.78rem;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
      </div>
      <div id="wofb" style="margin-top:8px;font-size:0.8rem"></div>
    </div>
    <button onclick="window.finLessonD('${unit.id}')" style="width:100%;padding:14px;border-radius:12px;background:linear-gradient(135deg,#d4456a,#f0a500);border:none;color:#fff;font-size:0.95rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Expresión oral & escrita yakunlash</button>`;
}
...
window.chkWO = function () {
    const fb = $id('wofb');
    if (!woAns.length) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> ¡Ordena las palabras!</span>'; return; }
    if (woAns.join(' ').toLowerCase() === (window.__woCorrect || '').toLowerCase()) {
        if (fb) fb.innerHTML = '<span style="color:#2ecc71"><i class="fa-solid fa-trophy" style="color:#f5c842"></i> ¡Perfecto!</span>';
        awardXP(15, 'writing'); lScore++;
    } else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Correcto: <em>${window.__woCorrect}</em></span>`; }
    lTotal++;
};
window.togMic = function (idx) {
    ...
    if (lessonMics[idx]) { try { lessonMics[idx].stop(); } catch (e) { } lessonMics[idx] = null; if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Hablar'; return; }
    ...
    rec.onerror = e => {
        if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Hablar'; lessonMics[idx] = null;
    ...
    rec.onend = () => { if (btn) btn.innerHTML = '<i class="fa-solid fa-microphone" style="color:#f87171"></i> Hablar'; if (st) st.innerHTML = '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Grabado'; lessonMics[idx] = null; };
    try {
        rec.start(); lessonMics[idx] = rec;
        if (btn) btn.innerHTML = '⏹ Parar';
        if (st) st.innerHTML = '<i class="fa-solid fa-circle" style="color:#ef4444"></i> Grabando...';
    } catch (e) {
        if (tr) tr.innerHTML = `<textarea style="width:100%;padding:8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#fff5f0;font-family:inherit" id="man${idx}" placeholder="Escribe aquí..."></textarea>`;
    }
};
window.aiSpk = async function (idx, topic) {
    ...
    if (!text) { if (fb) fb.innerHTML = '<span style="color:#f5c842"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> ¡Primero habla!</span>'; return; }
    if (fb) fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> Evaluando...';
    const r = await callAI(`Speaking baholash ispancha. Mavzu: "${topic}". O'quvchi: "${text}".\n${NATIVE_LANG} tilida (javob shu tilda bo'lsin): 1) <i class="fa-solid fa-circle-check" style="color:#34d399"></i> Yaxshi tomonlar 2) <i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Xatoliklar 3) <i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Tuzatilgan variant 4) <i class="fa-solid fa-star" style="color:#f5c842"></i> /10`, 700);
    if (fb) fb.innerHTML = esc(r).replace(/\n/g, '<br>');
    lScore++; lTotal++; awardXP(20, 'speaking');
};
window.markDone = function (idx) { lScore++; lTotal++; awardXP(10, 'speaking'); showToast('<i class="fa-solid fa-circle-check" style="color:#34d399"></i> ¡Completado!', 'success'); };
...
window.selfChk = function (min) {
    ...
    const w = ta.value.trim() ? ta.value.trim().split(/\s+/).length : 0;
    if (w >= min) { fb.innerHTML = `<span style="color:#2ecc71"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${w} palabras!</span>`; lScore++; awardXP(15, 'writing'); }
    else { fb.innerHTML = `<span style="color:#f87171"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Faltan ${min - w} palabras!</span>`; }
    lTotal++;
};
window.aiWrit = async function (title, words) {
    ...
    fb.innerHTML = '<i class="fa-solid fa-robot" style="color:#a78bfa"></i> Comprobando...';
    const r = await callAI(`Writing tekshirish ispancha. Mavzu: "${title}" (so'zlar: ${words}).\nMatn: "${ta.value.trim()}"\n${NATIVE_LANG} tilida (javob shu tilda bo'lsin): 1) Grammatika 2) Uslub 3) Tuzatilgan variant 4) DELE bali: /9`, 800);
...
async function saveLessonCompletion(unitId, lessonKey, score, total, xpEarned, coinEarned) {
    ...
    if (!CU) {
        ...
        showToast(`<i class="fa-solid fa-circle-check" style="color:#34d399"></i> +${xpEarned} XP +${coinEarned} <i class="fa-solid fa-coins" style="color:#f5c842"></i>`, 'success');
        return;
    }
    try {
        const updates = {
        ...
        if (allDone) {
            ...
            showToast(`<i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> Unidad completada! +50 XP +10 <i class="fa-solid fa-coins" style="color:#f5c842"></i> bonus!`, 'success');
        }
        await updateUserField(updates);
        UXP += xpEarned + (allDone ? 50 : 0);
        UCoin += coinEarned + (allDone ? 10 : 0);
        updateDisplays();
        showToast(`<i class="fa-solid fa-circle-check" style="color:#34d399"></i> +${xpEarned} XP +${coinEarned} <i class="fa-solid fa-coins" style="color:#f5c842"></i> guardado!`, 'success');
...
function showResult(lk, pct, xp, coin, unit, uid) {
    ...
    content.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="font-size:1.2rem;margin-bottom:20px">${pct >= 80 ? '<i class="fa-solid fa-trophy" style="color:#f5c842"></i> ¡Perfecto!' : pct >= 60 ? '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> ¡Muy bien!' : '<i class="fa-solid fa-dumbbell" style="color:#34d399"></i> ¡Inténtalo de nuevo!'}</div>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${nxt[lk] ? `<button onclick="window.openLesson('${uid}','${nxt[lk]}')" style="padding:12px;border-radius:12px;background:linear-gradient(135deg,#d4456a,#f0a500);border:none;color:#fff;font-size:0.9rem;font-weight:700;cursor:pointer;font-family:inherit"><i class="fa-solid fa-arrow-right"></i> Siguiente: ${lnames[nxt[lk]]}</button>` : `<div style="padding:14px;border-radius:12px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.25);color:#2ecc71;font-weight:700"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> ¡Unidad completada!</div>`}
        <button onclick="document.getElementById('unitModal').classList.remove('active');renderUnits()" style="padding:12px;border-radius:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:#fff5f0;cursor:pointer;font-size:0.9rem;font-family:inherit"><i class="fa-solid fa-house" style="color:#5b7cfa"></i> Volver a unidades</button>
      </div>
    </div>`;
    showXPPop(`+${xp} XP`);
...
function renderWords(reset = true) {
    ...
    slice.slice(wOff).forEach(w => {
        ...
        card.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
          ...
          <button onclick="window.spk('${w.e.replace(/'/g, "\\'")}',event)" style="background:none;border:none;cursor:pointer;font-size:1rem"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
        </div>
...
function openWModal(w) {
    ...
    c.innerHTML = `<div style="text-align:center;padding:20px">
      ...
      <div style="display:flex;gap:8px;justify-content:center">
        <button onclick="window.spk('${w.e.replace(/'/g, "\\'")}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(212,69,106,0.15);border:1px solid rgba(212,69,106,0.3);color:#d4456a;cursor:pointer;font-family:inherit"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i> Pronunciar</button>
        <button onclick="window.aiExWord('${w.e.replace(/'/g, "\\'")}',event)" style="padding:8px 20px;border-radius:10px;background:rgba(240,165,0,0.1);border:1px solid rgba(240,165,0,0.25);color:#f0a500;cursor:pointer;font-family:inherit"><i class="fa-solid fa-robot" style="color:#a78bfa"></i> AI (1 token)</button>
      </div>
...
window.checkQuizOpt = function (btn, chosen, type) {
    ...
    else { if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Correcto: ${type === 'es2uz' ? curQuizWord.u : curQuizWord.e}</span>`; }
...
window.selectMatch2 = function (el) {
    ...
    if (!matchSel1) {
        ...
        if (matchSel1.dataset.id === el.dataset.id) {
            ...
            if (matchMatched.length === matchPairs.length) { const fb = $id('matchFeedback'); if (fb) fb.innerHTML = '<span style="color:#2ecc71"><i class="fa-solid fa-champagne-glasses" style="color:#f5c842"></i> ¡Todos los pares!</span>'; }
        } else {
...
window.checkTyping = function () {
    ...
    if (val === w.e.toLowerCase()) {
        if (fb) fb.innerHTML = '<span style="color:#2ecc71"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ¡Correcto!</span>';
        ...
        if (fb) fb.innerHTML = `<span style="color:#ef4444"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Correcto: ${w.e}</span>`;
...
window.checkGrammar = function (chosen, ans, exp) {
    ...
    if (chosen === ans) {
        if (fb) fb.innerHTML = `<div style="color:#2ecc71;padding:10px;border-radius:10px;background:rgba(46,204,113,0.1)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ¡Correcto! ${exp}</div>`;
        grammarScore2++; const el = $id('grammarScore'); if (el) el.textContent = grammarScore2; awardXP(12, 'grammar');
    } else {
        if (fb) fb.innerHTML = `<div style="color:#ef4444;padding:10px;border-radius:10px;background:rgba(239,68,68,0.1)"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Incorrecto. Correcto: <b>${ans}</b>. ${exp}</div>`;
    }
};
...
window.sendChatMessage = async function () {
    ...
    try {
        ...
        if (!resp.ok) { appendChat('assistant', `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> AI xatolik: ${resp.status}`, false); return; }
        ...
        const tb = $id(typingId); if (tb) tb.remove();
        appendChat('assistant', `<i class="fa-solid fa-exclamation" style="color:#fbbf24"></i> Error: ${e.message || 'problema de red'}`, false);
    } finally {
...
window.clearChatHistory = async function () {
    ...
    if (c) c.innerHTML = `<div class="chat-msg ai-msg"><div class="chat-avatar"><i class="fa-solid fa-robot"></i></div><div class="chat-bubble">¡Hola! Historial borrado. ¡Empecemos una nueva conversación! <i class="fa-solid fa-face-smile" style="color:#f5c842"></i></div></div>`;
