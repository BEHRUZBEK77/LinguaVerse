...
// XSS himoyasi: foydalanuvchi kiritgan matnni innerHTML ichiga qo'yishdan
// oldin ekranlaymiz (masalan dictation input qiymati).
function esc(s) {
...
const LANG = {
    detect() {
        // 1) ENG ISHONCHLI YO'L: sahifaning o'zi window.LV_LANG orqali o'z
        //    tilini aniq e'lon qiladi (har bir *.html <head> ichida bitta
        //    qator bilan). Bu path/title bo'yicha taxmin qilishdan ko'ra
        //    ancha ishonchli — fayl nomi o'zgarsa ham ishlayveradi.
        if (window.LV_TARGET_LANG && LANG_KEYS.includes(window.LV_TARGET_LANG)) {
            return window.LV_TARGET_LANG;
        }

        // 2) ZAXIRA: agar sahifa window.LV_LANG'ni e'lon qilmagan bo'lsa
        //    (eski versiya), path/title bo'yicha taxmin qilamiz — lekin
        //    bu endi faqat fallback, asosiy manba emas.
        ...
        if (p.includes('turkish') || p.includes('turkey') || t.includes('Türkçe')) return 'turkish';
        if (p.includes('korean') || t.includes('한국어')) return 'korean';
        console.warn('[LANG.detect] window.LV_TARGET_LANG topilmadi va path/title ham mos kelmadi — "english" ga tushib qolindi. Bu sahifada <script>window.LV_TARGET_LANG=\'...\'</script> qo\'shilganini tekshiring.');
        return 'english';
    },
    cfg(lang) {
        const M = {
            ...
            turkish: { name: 'Turkish', flag: '🇹🇷', color: '#e30a17', c2: '#ffffff', tts: 'tr-TR', exam: 'TÖMER', native: 'Uzbek/Turkish' },
            korean: { name: 'Korean', flag: '🇰🇷', color: '#003478', c2: '#c60c30', tts: 'ko-KR', exam: 'TOPIK', native: 'Uzbek/Korean' }
        };
        return M[lang] || M.english;
    }
};
const LANG_KEYS = ['english', 'russian', 'german', 'french', 'spanish', 'arabic', 'japanese', 'chinese', 'turkish', 'korean'];
...
`;
    ...
    _buildPanel() {
        ...
        fab.id = 'ae-fab';
        fab.innerHTML = `<span class="fi"><i class="fa-solid fa-robot" style="color:#a78bfa"></i></span><span class="lt">AI Coach</span>`;
        ...
        <div class="ae-hd">
            ...
            <button class="ae-close-btn" onclick="window.__AE.toggle()"><i class="fa-solid fa-xmark" style="color:#f87171"></i></button>
        </div>
        <div class="ae-token-bar">
            <span class="ae-token-pill"><i class="fa-solid fa-ticket" style="color:#a78bfa"></i> ${this.userTokens}</span>
        ...
        <div class="ae-tabs">
            <div class="ae-tab on"  data-tab="plan"    onclick="window.__AE.nav('plan',this)"><i class="fa-solid fa-calendar-days" style="color:#60a5fa"></i><br>Plan</div>
            <div class="ae-tab"     data-tab="skills"  onclick="window.__AE.nav('skills',this)"><i class="fa-solid fa-chart-pie" style="color:#60a5fa"></i><br>Skills</div>
            <div class="ae-tab"     data-tab="lesson"  onclick="window.__AE.nav('lesson',this)"><i class="fa-solid fa-graduation-cap" style="color:#a78bfa"></i><br>Lesson</div>
            <div class="ae-tab"     data-tab="roadmap" onclick="window.__AE.nav('roadmap',this)"><i class="fa-solid fa-map" style="color:#94a3b8"></i><br>Map</div>
        </div>
    ...
    _renderPlan(body) {
        ...
        body.innerHTML = `
        <div class="ae-motive">
            <div class="ae-motive-greet">${p.greeting || 'Xush kelibsiz! <i class="fa-solid fa-dumbbell" style="color:#34d399"></i>'}</div>
            <div class="ae-motive-tip"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${p.motivationTip || ''}</div>
        </div>

        <div class="ae-card gold" style="display:flex;align-items:center;gap:10px;padding:10px 13px">
            <span><i class="fa-solid fa-bolt" style="color:#f5c842"></i></span>
            <div style="flex:1;font-size:.72rem;color:#f5c842">${p.funChallenge || 'Bugun 5 ta yangi so\'z o\'rganing!'}</div>
        </div>

        ${remaining === 0 ? `<div class="ae-limit-warn"><span><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i></span><div style="font-size:.75rem;color:#ef4444">Kunlik dars limiti (${lim.daily}) tugadi! Ertaga qayta keling yoki rejani yaxshilang.</div></div>` : ''}

        <div class="ae-sec"><span><i class="fa-solid fa-calendar-days" style="color:#60a5fa"></i></span> Bugungi reja <span class="ae-badge">${p.totalMinutes || 30} min</span></div>
        ${(p.tasks || []).map(t => `
        <div class="ae-task ae-priority-${t.priority?.[0] || 'm'} ${done.includes(t.order) ? 'done' : ''}"
            ...
            <div class="ae-task-meta">
                ...
                ${done.includes(t.order) ? '<div style="font-size:1rem;margin-top:2px"><i class="fa-solid fa-circle-check" style="color:#34d399"></i></div>' : ''}
            </div>
        </div>`).join('')}

        <div class="ae-sec"><span><i class="fa-solid fa-bullseye" style="color:#f87171"></i></span> Haftalik maqsad</div>
        <div class="ae-card" style="font-size:.78rem;color:#aaa;line-height:1.6">${p.weeklyGoal || ''}</div>

        <div style="margin-top:8px">
            <button class="ae-btn outline" onclick="window.__AE.regenPlan()"><i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Yangi reja yaratish</button>
        </div>
        `;
    },
    ...
    _renderSkills(body) {
        ...
        const c = this.langCfg.color;
        body.innerHTML = `
        <div class="ae-sec"><span><i class="fa-solid fa-chart-pie" style="color:#60a5fa"></i></span> Skill Map</div>
        ${Object.entries(s).map(([sk, val]) => {
            const cls = val < 35 ? 'w' : val < 65 ? 'm' : 's';
            const lbl = val < 35 ? '<i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i> Zaif' : val < 65 ? '<i class="fa-solid fa-chart-line" style="color:#34d399"></i> O\'rta' : '<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Kuchli';
        ...
        <div class="ae-sec"><span><i class="fa-solid fa-bolt" style="color:#f5c842"></i></span> Eng zaif ko'nikmalar</div>
        ...
        <div class="ae-card" style="display:flex;align-items:center;gap:10px;padding:11px 13px">
            <span style="font-size:1.2rem"><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i></span>
            <div style="flex:1">
                <div style="font-size:.82rem;font-weight:700;color:#e8ecff">${sk}</div>
                <div style="font-size:.68rem;color:#555">Score: ${val}%</div>
            </div>
            <button class="ae-btn sm outline" onclick="window.__AE.launchLesson('${sk}')">Train <i class="fa-solid fa-arrow-right"></i></button>
        </div>`).join('')}

        <div class="ae-sec"><span><i class="fa-solid fa-dumbbell" style="color:#34d399"></i></span> Kuchli tomonlar</div>
        ...
        <div class="ae-card green" style="display:flex;align-items:center;gap:10px;padding:10px 13px">
            <span><i class="fa-solid fa-circle-check" style="color:#34d399"></i></span>
        ...
        <div class="ae-sec"><span><i class="fa-solid fa-graduation-cap" style="color:#a78bfa"></i></span> Dars boshlash</div>
    ...
    _renderLesson(body) {
        if (!this.currentLesson) {
            ...
            <div class="ae-card accent" style="text-align:center;padding:24px">
                <div style="font-size:3rem;margin-bottom:10px"><i class="fa-solid fa-graduation-cap" style="color:#a78bfa"></i></div>
                ...
                <button class="ae-btn" onclick="window.__AE.launchLesson('${top}')"><i class="fa-solid fa-wand-magic-sparkles" style="color:#a78bfa"></i> ${top} darsi boshlash</button>
            </div>
            <div class="ae-sec"><span><i class="fa-solid fa-bullseye" style="color:#f87171"></i></span> Ko'nikma tanlang</div>
            ${['reading', 'listening', 'speaking', 'writing', 'vocabulary', 'grammar'].map(t => `
            <button class="ae-btn ghost" style="margin-bottom:7px" onclick="window.__AE.launchLesson('${t}')">
    ...
    async launchLesson(type) {
        ...
        if (!canCreate.ok) {
            body.innerHTML = `<div class="ae-limit-warn"><span><i class="fa-solid fa-ban" style="color:#f87171"></i></span><div style="font-size:.78rem;color:#ef4444"><b>Limit!</b> ${canCreate.reason}</div></div>
            <button class="ae-btn danger" onclick="window.location.href='../order.html'"><i class="fa-solid fa-arrow-up"></i> Rejani yaxshilash</button>`;
            return;
        }
        ...
        if (this.userPlan !== 'ultimate' && this.userPlan !== 'vip' && this.userTokens < D.own) {
            body.innerHTML = `<div class="ae-limit-warn"><span><i class="fa-solid fa-heart-crack" style="color:#f87171"></i></span><div style="font-size:.78rem;color:#ef4444">Tokenlar yetarli emas! Dars uchun <b>${D.own}</b> token kerak.</div></div>
        ...
        try {
            ...
            body.innerHTML = `<div class="ae-card red" style="text-align:center;padding:20px">
                <div style="font-size:1.5rem;margin-bottom:8px"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i></div>
    ...
    _lessonHeader(lesson, type) {
        ...
        <div class="ae-card accent" style="padding:13px">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
                ...
                <button class="ae-btn sm ghost" style="margin-left:auto;padding:5px 10px" onclick="window.__AE._backToLessonMenu()"><i class="fa-solid fa-arrow-left"></i> Orqaga</button>
            </div>
    ...
    _renderReadingLesson(body) {
        ...
        <div class="ae-card accent" style="padding:12px">
            ...
            <div style="display:flex;gap:7px;margin-bottom:10px">
                ...
                <button class="ae-audio-btn slow" onclick="window.__AE.speakPassageSlow()"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Sekin</button>
                <button class="ae-audio-btn stop" onclick="TTS.stop()">⏹ Stop</button>
            </div>
            <div style="font-size:.72rem;color:#555;margin-bottom:8px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> ~${l.passage?.wordCount || 200} so'z</div>
        </div>
        <div class="ae-passage" id="ae-passage-text">${esc(l.passage?.text || '').replace(/\n/g, '<br>')}</div>
        ...
        ${l.vocabulary.map(v => `<div class="ae-vocab">
            ...
            <button class="ae-vocab-spk" onclick="TTS.speak('${(v.word || '').replace(/'/g, "\\'")}')"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
        </div>`).join('')}` : ''}

        <div class="ae-sec"><span><i class="fa-solid fa-circle-question" style="color:#93c5fd"></i></span> Savollar (${qs.length} ta)</div>
        ` : ''}

        <!-- Current question -->
        ${cur ? `
        <div class="ae-ex" id="ae-cur-ex">
            <div class="ae-ex-label"><i class="fa-solid fa-circle-question" style="color:#93c5fd"></i> Savol ${this.exerciseIdx + 1} / ${qs.length}</div>
        ...
        <button class="ae-btn" id="ae-nxt" style="display:none" onclick="window.__AE.nextReadingQ()">Keyingi savol <i class="fa-solid fa-arrow-right"></i></button>
        ` : `
        <!-- Summary task -->
        ${l.summaryTask ? `
        ...
        <button class="ae-btn outline" onclick="window.__AE.checkSummary()"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshirish</button>
        ...
        <div class="ae-card" style="margin-top:8px">
            <div style="font-size:.8rem;font-weight:700;color:#e8ecff;margin-bottom:5px"><i class="fa-solid fa-comment" style="color:#94a3b8"></i> ${ct.question}</div>
            <div style="font-size:.72rem;color:#555;margin-bottom:8px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${ct.hint}</div>
            <textarea class="ae-textarea" id="ae-think-${i}" placeholder="Fikringizni yozing..." style="min-height:55px"></textarea>
        </div>`).join('')}

        <button class="ae-btn" style="margin-top:12px" onclick="window.__AE.finishLesson()"><i class="fa-solid fa-flag-checkered" style="color:#34d399"></i> Darsni yakunlash</button>
        `}
        `;
    },
    ...
    checkMC(btn, sel, correct, exp) {
        ...
        if (fb) {
            ...
            fb.innerHTML = ok ? `<i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri! ${exp}` : `<i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Noto'g'ri. To'g'ri javob: <b>${String.fromCharCode(65 + correct)}</b>. ${exp}`;
    ...
    _renderListeningLesson(body) {
        ...
        <div class="ae-audio-box" id="ae-script-${si}">
            <div class="ae-audio-title">${sc.type === 'dialogue' ? '<i class="fa-solid fa-comments" style="color:#5b7cfa"></i>' : '<i class="fa-solid fa-microphone-lines" style="color:#f87171"></i>'} ${sc.title || `Audio ${si + 1}`}
            ...
            <div class="ae-audio-btns">
                ...
                <button class="ae-audio-btn slow" onclick="window.__AE.playScriptSlow(${si})"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Sekin</button>
                <button class="ae-audio-btn stop" onclick="TTS.stop()">⏹</button>
                <button class="ae-audio-btn play" onclick="window.__AE.toggleScriptText(${si})" style="background:rgba(255,255,255,.05);border-color:rgba(255,255,255,.12);color:#aaa"><i class="fa-solid fa-file-lines" style="color:#94a3b8"></i> Matn</button>
            </div>
            <div class="ae-audio-text" id="ae-script-text-${si}">${esc(sc.text || '').replace(/\n/g, '<br>')}</div>
        </div>`).join('')}

        <!-- Current question -->
        ${cur ? `
        <div class="ae-ex" id="ae-cur-ex">
            <div class="ae-ex-label"><i class="fa-solid fa-headphones" style="color:#5b7cfa"></i> Savol ${this.exerciseIdx + 1} / ${allQs.length + 1} — Script ${cur.scriptIdx + 1}</div>
            <div class="ae-ex-q">${cur.q}</div>
            ${(cur.opts || []).map((opt, i) => `
        ...
        <button class="ae-btn" id="ae-nxt" style="display:none" onclick="window.__AE.nextListenQ(${allQs.length})">Keyingi <i class="fa-solid fa-arrow-right"></i></button>
        ` : isDictation ? `
        <!-- Dictation exercise -->
        <div class="ae-sec"><span><i class="fa-solid fa-pen-nib"></i></span> Diktant</div>
        <div class="ae-audio-box">
            <div class="ae-audio-title"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> Eshitib yozing</div>
            ...
            <div class="ae-audio-btns">
                ...
                <button class="ae-audio-btn slow" onclick="TTS.speakSlow('${(l.dictation?.sentence || '').replace(/'/g, "\\'")}')"><i class='fa-solid fa-gauge-simple' style='color:#94a3b8'></i> Sekin</button>
            </div>
        </div>
        <textarea class="ae-textarea" id="ae-dict-input" placeholder="Eshitganni yozing..."></textarea>
        <button class="ae-btn outline" onclick="window.__AE.checkDictation('${(l.dictation?.sentence || '').replace(/'/g, "\\'")}')"><i class='fa-solid fa-check' style='color:#34d399'></i> Tekshirish</button>
        <div id="ae-dict-fb" class="ae-feedback" style="display:none"></div>
        <button class="ae-btn" style="margin-top:10px" id="ae-nxt" style="display:none" onclick="window.__AE.finishLesson()"><i class="fa-solid fa-flag-checkered" style="color:#34d399"></i> Yakunlash</button>
        ` : `<button class="ae-btn" onclick="window.__AE.finishLesson()"><i class="fa-solid fa-flag-checkered" style="color:#34d399"></i> Darsni yakunlash</button>`}

        <!-- Phonetics -->
        ${(l.phonetics || []).length && this.exerciseIdx === 0 ? `
        <div class="ae-sec"><span><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></span> Talaffuz</div>
        ${l.phonetics.map(ph => `
        <div class="ae-card" style="padding:11px">
            ...
            <div style="display:flex;gap:6px;flex-wrap:wrap">
                ${(ph.examples || []).map(w => `<button class="ae-badge" style="cursor:pointer;border:none;background:${this.langCfg.color}18;padding:4px 10px" onclick="TTS.speak('${w}')">${w} <i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>`).join('')}
            </div>
        </div>`).join('')}` : ''}
        `;
    },
    ...
    checkDictation(correct) {
        ...
        fb.innerHTML = `<div>To'g'ri: ${hl}</div><div style="margin-top:4px">Sizning: ${esc(inp.value)}</div><div style="font-weight:700;margin-top:4px">${pct}% to'g'ri</div>`;
    ...
    _renderSpeakingLesson(body) {
        ...
        ${this.speakTaskIdx === 0 ? `
        <div class="ae-sec"><span><i class="fa-solid fa-comments" style="color:#5b7cfa"></i></span> Foydali iboralar</div>
        ${(l.phrases || []).map(ph => `
        <div class="ae-vocab">
            <div style="flex:1">
            ...
            <button class="ae-vocab-spk" onclick="TTS.speak('${(ph.phrase || '').replace(/'/g, "\\'")}')"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
        </div>`).join('')}

        <!-- Warmup -->
        ${l.warmup ? `
        <div class="ae-sec"><span><i class="fa-solid fa-fire" style="color:#fb923c"></i></span> Isitish mashqi</div>
        <div class="ae-card accent">
            ...
            <div style="font-size:.75rem;color:#555;margin-top:6px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${l.warmup.uzbekHint}</div>
        </div>` : ''}` : ''}
        ...
        <div class="ae-sec"><span><i class="fa-solid fa-font" style="color:#60a5fa"></i></span> Talaffuz</div>
        ${l.pronunciation.map(p => `
        <div class="ae-card" style="padding:11px">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
                <button class="ae-vocab-spk" style="font-size:1rem" onclick="TTS.speak('${(p.word || '').replace(/'/g, "\\'")}')"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
                <span style="font-size:.9rem;font-weight:800;color:#e8ecff">${p.word}</span>
                <span style="font-size:.72rem;color:#555">${p.ipa}</span>
            </div>
            <div style="font-size:.72rem;color:#ef4444;margin-bottom:2px"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> Xato: ${p.common_mistake}</div>
            <div style="font-size:.72rem;color:#10b981"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri: ${p.fix}</div>
        </div>`).join('')}` : ''}
        <button class="ae-btn" onclick="window.__AE.finishLesson()"><i class="fa-solid fa-flag-checkered" style="color:#34d399"></i> Darsni yakunlash</button>
        `}
        `;
    ...
    _renderSpeakTask(task) {
        ...
        const header = `<div class="ae-sec"><span>${{ repeat: '<i class="fa-solid fa-repeat"></i>', describe: '<i class="fa-solid fa-pen"></i>', roleplay: '<i class="fa-solid fa-masks-theater"></i>', opinion: '<i class="fa-solid fa-comment-dots"></i>', storytelling: '<i class="fa-solid fa-book-open"></i>' }[task.type] || '<i class="fa-solid fa-microphone"></i>'}</span> 
            ...
            <button class="ae-btn sm ghost" style="margin-left:auto" onclick="window.__AE.nextSpeakTask()">O'tkazib yuborish <i class="fa-solid fa-arrow-right"></i></button>
        </div>`;
        ...
        if (task.type === 'repeat') {
            ...
            <div class="ae-card">
                ...
                <div style="font-size:.72rem;color:${c};margin-bottom:10px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${task.tip}</div>
                <div class="ae-audio-btns">
                    <button class="ae-audio-btn play" onclick="TTS.speak('${(task.sentence || '').replace(/'/g, "\\'")}')">▶ Eshitish</button>
                    <button class="ae-audio-btn slow" onclick="TTS.speakSlow('${(task.sentence || '').replace(/'/g, "\\'")}')"><i class="fa-solid fa-gauge-simple" style="color:#94a3b8"></i> Sekin</button>
                    ${this._micButton(idx)}
                </div>
                <div id="ae-mic-transcript-${idx}" style="font-size:.78rem;color:#e8ecff;padding:6px;min-height:20px"></div>
                <div style="display:flex;gap:7px;margin-top:8px">
                    <button class="ae-btn sm outline" onclick="window.__AE.markSpeakDone(${idx},true)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Bajarildi</button>
                    <button class="ae-btn sm ghost" onclick="window.__AE.nextSpeakTask()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>
                </div>
            </div>`;
        }
        if (task.type === 'describe') {
            ...
            <div class="ae-card">
                <div style="font-size:.78rem;color:#aaa;margin-bottom:8px">${task.instruction}</div>
                <div style="font-size:.85rem;font-weight:700;color:#e8ecff;margin-bottom:8px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> ${task.prompt}</div>
                <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px">
                    ${(task.keyWords || []).map(w => `<button class="ae-badge" style="cursor:pointer" onclick="TTS.speak('${w}')">${w} <i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>`).join('')}
                </div>
                ...
                <div id="ae-mic-transcript-${idx}" style="font-size:.78rem;color:#e8ecff;padding:6px;min-height:20px"></div>
                <button class="ae-btn outline" style="margin-top:8px" onclick="window.__AE.markSpeakDone(${idx},true)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Bajarildi</button>
            </div>`;
        }
        if (task.type === 'roleplay') {
            ...
            <div class="ae-card">
                ...
                <div style="padding:10px;background:rgba(255,255,255,.03);border-radius:9px;margin-bottom:10px">
                    <div style="font-size:.75rem;color:#555;margin-bottom:3px"><i class="fa-solid fa-masks-theater" style="color:#a78bfa"></i> Ssenariy</div>
                ...
                <button class="ae-btn outline" style="margin-top:8px" onclick="window.__AE.markSpeakDone(${idx},true)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Yakunlash</button>
            </div>`;
        }
        ...
        return `${header}<button class="ae-btn" onclick="window.__AE.nextSpeakTask()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>`;
    },

    _micButton(idx) {
        return `<button class="ae-mic" id="ae-mic-${idx}" onclick="window.__AE.toggleMic(${idx})" style="margin-top:8px"><i class="fa-solid fa-microphone" style="color:#f87171"></i></button>`;
    },
    ...
    _renderWritingLesson(body) {
        ...
        <div class="ae-sec"><span><i class="fa-solid fa-fire" style="color:#fb923c"></i></span> Isitish</div>
        <div class="ae-card">
            ...
            <div style="margin-bottom:8px">
                ...
                <div style="font-size:.7rem;color:#555;margin-bottom:4px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> ${s.hint}</div>
        ...
        <div class="ae-card">
            ...
            ${(l.grammarFocus.examples || []).map(e => `<div style="font-size:.75rem;color:#e8ecff;margin-bottom:3px"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${e}</div>`).join('')}
            ${(l.grammarFocus.common_errors || []).map(e => `<div style="font-size:.75rem;color:#ef4444;margin-bottom:3px"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> ${e}</div>`).join('')}
        </div>` : ''}` : ''}
    ...
    _renderWriteTask(task) {
        ...
        const header = `<div class="ae-sec"><span><i class="fa-solid fa-pen-nib"></i></span> Vazifa ${idx + 1}/${tasks.length}: ${task.title}
            <button class="ae-btn sm ghost" style="margin-left:auto" onclick="window.__AE.nextWriteTask()">O'tkazish <i class="fa-solid fa-arrow-right"></i></button>
        </div>`;
        ...
        if (task.type === 'sentence_building') {
            ...
            <div class="ae-card">
                ...
                <div style="display:flex;gap:7px;margin-top:5px">
                    <button class="ae-btn sm outline" onclick="window.__AE.checkSentence('ae-sb-input','${(task.correctSentence || '').replace(/'/g, "\\'")}')"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
        ...
        if (task.type === 'paragraph') {
            ...
            <div class="ae-card">
                <div style="font-size:.78rem;color:#aaa;margin-bottom:8px">${task.instruction}</div>
                <div style="font-size:.85rem;font-weight:700;color:#e8ecff;margin-bottom:8px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> "${task.topic}"</div>
                ...
                <div style="display:flex;gap:7px">
                    <button class="ae-btn sm outline" onclick="window.__AE.checkWordCount('ae-para-input',${task.minWords},'ae-para-fb')"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
        ...
        if (task.type === 'email') {
            ...
            <div class="ae-card">
                ...
                <div style="display:flex;gap:7px">
                    <button class="ae-btn sm outline" onclick="window.__AE.markWriteDone(${idx},true)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Bajarildi</button>
        ...
        if (task.type === 'essay') {
            ...
            <div class="ae-card">
                ...
                <div style="display:flex;gap:7px">
                    <button class="ae-btn sm outline" onclick="window.__AE.checkWordCount('ae-essay-input',${task.minWords},'ae-essay-fb')"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
        ...
        if (task.type === 'correction') {
            ...
            <div>
                ...
                <div class="ae-card" style="margin-bottom:8px">
                    <div style="font-size:.8rem;color:#ef4444;margin-bottom:4px"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> ${s.wrong}</div>
                    <input class="ae-input" id="ae-corr-${i}" data-correct="${s.correct}" placeholder="To'g'risi...">
                    <div style="display:flex;gap:6px">
                        <button class="ae-btn sm outline" onclick="window.__AE.checkCorrection('ae-corr-${i}','ae-corr-fb-${i}','${(s.correct || '').replace(/'/g, "\\'")}','${(s.uzbek_exp || '').replace(/'/g, "\\'")}')"><i class="fa-solid fa-check" style="color:#34d399"></i></button>
                    </div>
                    <div id="ae-corr-fb-${i}" class="ae-feedback" style="display:none"></div>
                </div>`).join('')}
                <button class="ae-btn outline" style="margin-top:8px" onclick="window.__AE.markWriteDone(${idx},true)"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> Bajarildi</button>
            </div>`;
        }
        return `${header}<button class="ae-btn" onclick="window.__AE.nextWriteTask()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>`;
    },
    ...
    checkSentence(inputId, correct) {
        ...
        fb.style.display = 'block';
        fb.className = `ae-feedback ${ok ? 'ok' : 'fail'}`;
        fb.innerHTML = ok ? `<i class="fa-solid fa-circle-check" style="color:#34d399"></i> Ajoyib! To'g'ri jumla.` : `<i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: <b>${correct}</b>`;
    ...
    checkCorrection(inputId, fbId, correct, uzbekExp) {
        ...
        fb.style.display = 'block';
        fb.className = `ae-feedback ${ok ? 'ok' : 'fail'}`;
        fb.innerHTML = ok ? `<i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri!` : `<i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: <b>${correct}</b> — ${uzbekExp}`;
    ...
    _renderVocabLesson(body) {
        ...
        <!-- Exercises -->
        <div class="ae-sec"><span><i class="fa-solid fa-pencil" style="color:#60a5fa"></i></span> Mashqlar</div>
        ${cur ? this._renderVocabExercise(cur) : `
        <!-- Idioms -->
        ${(l.idioms || []).length ? `
        <div class="ae-sec"><span><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i></span> Idiomlar</div>
        ${l.idioms.map(id => `
        <div class="ae-card" style="padding:11px">
            ...
            <button class="ae-vocab-spk" onclick="TTS.speak('${(id.expression || '').replace(/'/g, "\\'")}')"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i></button>
        </div>`).join('')}` : ''}

        ${l.memoryTrick ? `
        <div class="ae-card gold" style="padding:11px">
            <div style="font-size:.65rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:#f5c842;margin-bottom:4px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> YODLASH USULI</div>
    ...
    _renderVocabExercise(ex) {
        if (ex.type === 'match') {
            return `<div class="ae-ex">
                <div class="ae-ex-label"><i class="fa-solid fa-link" style="color:#60a5fa"></i> Juftlash</div>
                ...
                <button class="ae-btn" id="ae-nxt" style="display:none;margin-top:8px" onclick="window.__AE.nextVocabEx()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>
            </div>`;
        }
        if (ex.type === 'fill') {
            return `<div class="ae-ex">
                <div class="ae-ex-label"><i class="fa-solid fa-pencil" style="color:#60a5fa"></i> Bo'sh to'ldirish</div>
                <div class="ae-ex-q">${ex.instruction}</div>
                ...
                <div style="margin-bottom:10px">
                    ...
                    <div style="display:flex;gap:6px">
                        ...
                        <button class="ae-btn sm outline" onclick="window.__AE.checkFillVocab('ae-fill-${i}','ae-fill-fb-${i}')"><i class="fa-solid fa-check" style="color:#34d399"></i></button>
                    </div>
                    <div id="ae-fill-fb-${i}" class="ae-feedback" style="display:none"></div>
                </div>`).join('')}
                <button class="ae-btn outline" style="margin-top:6px" onclick="window.__AE.nextVocabEx()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>
            </div>`;
        }
        return `<div class="ae-ex"><div class="ae-ex-q">${ex.instruction}</div>
            <button class="ae-btn outline" onclick="window.__AE.nextVocabEx()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>
        </div>`;
    },
    ...
    checkFillVocab(inputId, fbId) {
        ...
        fb.style.display = 'block';
        fb.className = `ae-feedback ${ok ? 'ok' : 'fail'}`;
        fb.innerHTML = ok ? `<i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri!` : `<i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: <b>${inp.dataset.ans}</b>`;
        if (ok) { this.score++; } this.total++;
    },
    nextVocabEx() {
        this.exerciseIdx++;
    ...
    _renderGrammarLesson(body) {
        ...
        <div class="ae-card accent" style="padding:13px;margin-bottom:10px">
            <div style="font-size:.65rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:${c};margin-bottom:6px"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> QOIDA</div>
            ...
            <button class="ae-audio-btn play" onclick="TTS.speak('${(l.rule || '').replace(/'/g, "\\'")}')"><i class='fa-solid fa-volume-high' style='color:#5b7cfa'></i> Qoidani eshitish</button>
        </div>

        <!-- Examples -->
        <div class="ae-sec"><span><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i></span> Misollar</div>
        ${(l.examples || []).map(ex => `
        <div class="ae-vocab" style="align-items:flex-start">
            ...
            <button class="ae-vocab-spk" onclick="TTS.speak('${(ex[Object.keys(ex)[0]] || '').replace(/'/g, "\\'")}')"><i class="fa-solid fa-volume-high" style="color:#5b7cfa"></i></button>
        </div>`).join('')}

        <!-- Common errors -->
        ${(l.commonErrors || []).length ? `
        <div class="ae-sec"><span><i class="fa-solid fa-triangle-exclamation" style="color:#fbbf24"></i></span> Keng tarqalgan xatolar</div>
        ${l.commonErrors.map(ce => `
        <div class="ae-card" style="padding:11px">
            <div style="font-size:.76rem;color:#ef4444;margin-bottom:3px"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> ${ce.wrong}</div>
            <div style="font-size:.76rem;color:#10b981;margin-bottom:3px"><i class="fa-solid fa-circle-check" style="color:#34d399"></i> ${ce.correct}</div>
        ...
        ${l.quickTip ? `<div class="ae-card gold" style="padding:10px;margin-top:10px"><div style="font-size:.65rem;font-weight:800;color:#f5c842;margin-bottom:3px"><i class="fa-solid fa-lightbulb" style="color:#f5c842"></i> MASLAHAT</div><div style="font-size:.76rem;color:#aaa">${l.quickTip}</div></div>` : ''}
        <button class="ae-btn" style="margin-top:12px" onclick="window.__AE.finishLesson()"><i class="fa-solid fa-flag-checkered" style="color:#34d399"></i> Darsni yakunlash</button>
        `}
        `;
    },
    ...
    _renderGrammarItem(item) {
        ...
        if (item.type === 'multiple_choice') {
            return `<div class="ae-ex" id="ae-cur-ex">
                <div class="ae-ex-label"><i class="fa-solid fa-pen-to-square" style="color:#60a5fa"></i> Test — savol ${idx + 1}</div>
                ...
                <div id="ae-fb" class="ae-feedback" style="display:none"></div>
                <button class="ae-btn" id="ae-nxt" style="display:none;margin-top:8px" onclick="window.__AE.nextGrammarItem()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>
            </div>`;
        }
        if (item.type === 'transform') {
            return `<div class="ae-ex">
                <div class="ae-ex-label"><i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> O'zgartirish — ${idx + 1}</div>
                ...
                <div style="display:flex;gap:7px">
                    <button class="ae-btn sm outline" onclick="window.__AE.checkTransform('${(item.answer || '').replace(/'/g, "\\'")}')"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
        ...
        if (item.type === 'error_correction') {
            return `<div class="ae-ex">
                <div class="ae-ex-label"><i class="fa-solid fa-magnifying-glass" style="color:#94a3b8"></i> Xatoni toping — ${idx + 1}</div>
                ${item.instruction ? `<div style="font-size:.72rem;color:#888;margin-bottom:7px">${item.instruction}</div>` : ''}
                <div style="font-size:.84rem;color:#ef4444;margin-bottom:8px;padding:8px;background:rgba(239,68,68,.07);border-radius:8px"><i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> ${item.wrong}</div>
                <input class="ae-input" id="ae-ec-inp" placeholder="To'g'ri variant...">
                <button class="ae-btn sm outline" onclick="window.__AE.checkTransform('${(item.correct || '').replace(/'/g, "\\'")}')"><i class="fa-solid fa-check" style="color:#34d399"></i> Tekshir</button>
                <div id="ae-fb" class="ae-feedback" style="display:none"></div>
                <button class="ae-btn" id="ae-nxt" style="display:none;margin-top:8px" onclick="window.__AE.nextGrammarItem()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>
            </div>`;
        }
        return `<button class="ae-btn outline" onclick="window.__AE.nextGrammarItem()">Keyingi <i class="fa-solid fa-arrow-right"></i></button>`;
    },
    ...
    checkTransform(correct) {
        ...
        fb.style.display = 'block';
        fb.className = `ae-feedback ${ok ? 'ok' : 'fail'}`;
        fb.innerHTML = ok ? `<i class="fa-solid fa-circle-check" style="color:#34d399"></i> To'g'ri!` : `<i class="fa-solid fa-circle-xmark" style="color:#f87171"></i> To'g'ri: <b>${correct}</b>`;
    ...
    async finishLesson() {
        ...
        body.innerHTML = `
        <div class="ae-result">
            <div style="font-size:4rem;margin-bottom:10px">${pct >= 80 ? '<i class="fa-solid fa-trophy" style="color:#f5c842"></i>' : pct >= 60 ? '<i class="fa-solid fa-circle-check" style="color:#34d399"></i>' : '<i class="fa-solid fa-dumbbell" style="color:#34d399"></i>'}</div>
            ...
            <div style="padding:12px;border-radius:11px;background:${c}10;border:1px solid ${c}22;margin-bottom:16px;font-size:.78rem;color:#aaa;line-height:1.55">
                ${pct >= 80 ? 'Ajoyib natija! Siz bu ko\'nikmani juda yaxshi o\'rganmoqdasiz. Davom eting! <i class="fa-solid fa-rocket" style="color:#a78bfa"></i>' :
                pct >= 60 ? 'Yaxshi ish! Biroz mashq qilib, siz yanada yaxshilanasiz. Ko\'proq urinib ko\'ring! <i class="fa-solid fa-dumbbell" style="color:#34d399"></i>' :
                    'Tushkunlikka tushmang! Har mashq sizi yaxshiroq qiladi. Qayta urinib ko\'ring! <i class="fa-solid fa-seedling" style="color:#34d399"></i>'}
            </div>

            <button class="ae-btn" onclick="window.__AE._resetAndLesson('${skill}')"><i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Yana mashq</button>
            <button class="ae-btn ghost" onclick="window.__AE._backToLessonMenu()"><i class="fa-solid fa-arrow-left"></i> Boshqa dars</button>
            <button class="ae-btn ghost" onclick="window.__AE.nav('plan', document.querySelector('[data-tab=\\'plan\\']'))"><i class="fa-solid fa-calendar-days" style="color:#60a5fa"></i> Rejaga qaytish</button>
        </div>`;
    ...
    _renderRoadmap(body) {
        if (!this.roadmap) {
            body.innerHTML = `
            <div class="ae-card accent" style="text-align:center;padding:24px">
                <div style="font-size:3rem;margin-bottom:12px"><i class="fa-solid fa-map" style="color:#94a3b8"></i></div>
                ...
                <button class="ae-btn" onclick="window.__AE.genRoadmap()"><i class="fa-solid fa-map" style="color:#94a3b8"></i> Roadmap yaratish</button>
            </div>`;
            return;
        }
        ...
        <div class="ae-card accent" style="padding:12px;margin-bottom:12px">
            <div style="display:flex;gap:8px;align-items:center">
                <div style="flex:1">
                    <div style="font-size:.65rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:${c}"><i class="fa-solid fa-chart-line" style="color:#34d399"></i> DARAJA</div>
                    <div style="font-size:.88rem;font-weight:700;color:#e8ecff">${r.currentLevel} <i class="fa-solid fa-arrow-right"></i> ${r.targetLevel}</div>
                </div>
        ...
        <div class="ae-sec"><span><i class="fa-solid fa-calendar-days" style="color:#60a5fa"></i></span> Haftalik reja</div>
        ${(r.weeks || []).map(w => `
        <div class="ae-rm-week">
            <div class="ae-rm-wnum">${w.emoji || '<i class="fa-solid fa-calendar-days" style="color:#60a5fa"></i>'} Hafta ${w.week}</div>
            <div class="ae-rm-wfocus">${w.focus}</div>
            <div class="ae-rm-topics">
                ${(w.topics || []).map(t => `<span class="ae-rm-topic">${t}</span>`).join('')}
            </div>
            <div style="font-size:.74rem;color:#888;margin-bottom:4px"><i class="fa-solid fa-bullseye" style="color:#f87171"></i> ${w.milestone}</div>
            <div style="font-size:.7rem;color:#f5c842"><i class="fa-solid fa-star" style="color:#f5c842"></i> Maqsad: ${w.xpTarget} XP • ${w.lessonsPerDay} dars/kun</div>
        </div>`).join('')}

        <div class="ae-sec"><span><i class="fa-solid fa-calendar-days" style="color:#60a5fa"></i></span> Jadval</div>
        <div class="ae-card" style="padding:12px">
            <div style="font-size:.78rem;color:#ccc;margin-bottom:5px"><i class="fa-solid fa-calendar" style="color:#60a5fa"></i> Haftada ${r.schedule?.daysPerWeek || 5} kun</div>
        ...
        <div class="ae-card gold" style="padding:11px;margin-top:8px">
            <div style="font-size:.65rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:#f5c842;margin-bottom:4px"><i class="fa-solid fa-bullseye" style="color:#f87171"></i> UZOQ MUDDATLI MAQSAD</div>
            <div style="font-size:.78rem;color:#aaa">${r.longTermGoal}</div>
        </div>`: ''}

        ${r.examTip ? `
        <div class="ae-card" style="padding:11px;margin-top:8px">
            <div style="font-size:.65rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:${c};margin-bottom:4px"><i class="fa-solid fa-clipboard-list" style="color:#94a3b8"></i> ${this.langCfg.exam} MASLAHAT</div>
            <div style="font-size:.78rem;color:#aaa">${r.examTip}</div>
        </div>`: ''}

        <div style="margin-top:14px">
            <button class="ae-btn ghost" onclick="window.__AE.genRoadmap()"><i class="fa-solid fa-rotate" style="color:#5b7cfa"></i> Roadmapni yangilash</button>
        </div>`;
    },
