...
async function azureTTS(text, lang, rate) {
  const key = keys.get('AZURE_SPEECH_KEY');
...
async function googleTTS(body) {
  if (!keys.get('GOOGLE_TTS_KEYS')) return null;
  const TTS_KEYS = keys.get('GOOGLE_TTS_KEYS').split(',').map(k => k.trim()).filter(Boolean);
...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === 'GET') {
    const region = (keys.get('AZURE_SPEECH_REGION') || '(not set)');
    const status = {
      ok: true,
      azure: { keySet: !!keys.get('AZURE_SPEECH_KEY'), region },
      google: { keySet: !!keys.get('GOOGLE_TTS_KEYS') }
    };
  ...
  // Client-side chaqiruv topilmadi (azure-speech.js orqali ishlatilayotganga
  // o'xshaydi), lekin baribir pullik TTS provayderlariga ulanadi — tezlik
  // chekloviga olindi.
  const rl = await ratelimit.checkRateLimit(`tts:${ratelimit.clientIp(event)}`, { windowMs: 60_000, max: 25 });
  if (!rl.ok) {
