...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === 'GET') {
    return {
      ...
      body: JSON.stringify({ ok: true, hasKey: !!keys.get('GROQ_API_KEY') })
    };
  }
  if (event.httpMethod !== 'POST') return { statusCode: 405, headers: CORS, body: 'Method Not Allowed' };

  // Ovoz yozib yuborish og'ir/qimmat amal — IP bo'yicha tezlik cheklovi
  // qo'shildi (speaking-coach.html orqali chaqiriladi).
  const rl = await ratelimit.checkRateLimit(`whisper:${ratelimit.clientIp(event)}`, { windowMs: 60_000, max: 15 });
  if (!rl.ok) {
  ...
  try {
    ...
    const resp = await fetch('https://api.groq.com/openai/v1/audio/transcriptions', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${keys.get('GROQ_API_KEY')}` },
      body: form
    });
